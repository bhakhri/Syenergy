...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = BankBranchManager::getInstance()->getBankBranch(' WHERE UCASE(branchName)="'.add_slashes(trim(strtoupper($REQUEST_DATA['branchName']))).'" AND bankBranchId!='.$REQUEST_DATA['bankBranchId']);

		if(trim($foundArray[0]['branchName'])=='') {  //DUPLICATE Bank NAME CHECK
        
			$foundArray2 = BankBranchManager::getInstance()->getBankBranchAbbr(' WHERE UCASE(branchAbbr) = "'.add_slashes(trim(strtoupper($REQUEST_DATA['branchAbbr']))).'" AND bankBranchId!='.$REQUEST_DATA['bankBranchId']);
			if(trim($foundArray2[0]['branchAbbr'])=='') {  //DUPLICATE Bank YEAR CHECK
				$foundArray3 = BankBranchManager::getInstance()->getBankBranchAccountNumber(' WHERE UCASE(accountNumber) = "'.add_slashes(strtoupper($REQUEST_DATA['accountNumber'])).'" AND bankBranchId!='.$REQUEST_DATA['bankBranchId']);
				if (trim($foundArray3[0]['accountNumber'] == '')) {
					$returnStatus = BankBranchManager::getInstance()->editBankBranch($REQUEST_DATA['bankBranchId']);
