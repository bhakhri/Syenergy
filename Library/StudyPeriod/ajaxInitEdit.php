<?php
//-------------------------------------------------------
// THIS FILE IS USED TO EDIT A studyPeriod
//
//
...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = StudyPeriodManager::getInstance()->getStudyPeriod(' WHERE UCASE(periodValue)="'.add_slashes(strtoupper($REQUEST_DATA['periodValue'])).'" AND periodicityId="'.add_slashes($REQUEST_DATA['periodicityId']).'" AND studyPeriodId!='.$REQUEST_DATA['studyPeriodId']);
        if(trim($foundArray[0]['periodValue'])=='') {  //DUPLICATE CHECK
            $returnStatus = StudyPeriodManager::getInstance()->editStudyPeriod($REQUEST_DATA['studyPeriodId']);
