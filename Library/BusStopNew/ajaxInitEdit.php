...
$busStopManager = BusStopManagerNew::getInstance();
    ...
    if ($errorMessage == '' && (!isset($REQUEST_DATA['stopAbbr']) || trim($REQUEST_DATA['stopAbbr']) == '')) {
     ...
     require_once(MODEL_PATH . "/BusStopManagerNew.inc.php");
        $foundArray = $busStopManager->getBusStop(' AND (UCASE(bs.stopName)="'.add_slashes(strtoupper($REQUEST_DATA['stopName'])).'" OR UCASE(bs.stopAbbr)="'.add_slashes(strtoupper($REQUEST_DATA['stopAbbr'])).'") AND bs.busStopId!='.$REQUEST_DATA['busStopId']);
    ...
    if (trim($errorMessage) == '') {
    	if(SystemDatabaseManager::getInstance()->startTransaction()) {
			...
			$returnStatus = $busStopManager->editBusStop($REQUEST_DATA['busStopId'],$busStopCityId);
			if($returnStatus === false) {
				echo FAILURE;
				die;
			}
