...
UtilityManager::headerNoCache();
   ...
   /* if (!isset($REQUEST_DATA['degreeId']) || trim($REQUEST_DATA['degreeId']) == '') {
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = TestTypeManager::getInstance()->getTestType(' WHERE UCASE(testTypeCode)="'.add_slashes(strtoupper(trim($REQUEST_DATA['testtypeCode']))).'" AND testTypeId!='.$REQUEST_DATA['testtypeId']);
        if(trim($foundArray[0]['testTypeCode'])=='') {  //DUPLICATE CHECK
            $returnStatus = TestTypeManager::getInstance()->editTestType($REQUEST_DATA['testtypeId']);
