...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = DisciplineManager::getInstance()->getDiscipline(' AND sc.studentId='.$REQUEST_DATA['studentId'].' AND offenseId='.$REQUEST_DATA['offenseId'].' AND offenseDate="'.$REQUEST_DATA['offenseDate'].'"');
        if(trim($foundArray[0]['offenseDate'])=='') {  //DUPLICATE CHECK
            $returnStatus = DisciplineManager::getInstance()->addDiscipline();
