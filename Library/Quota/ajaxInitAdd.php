<?php
//-------------------------------------------------------
// THIS FILE IS USED TO ADD A QUOTA 
//
...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = QuotaManager::getInstance()->getQuota(' WHERE ( UCASE(quotaAbbr)="'.add_slashes(strtoupper(trim($REQUEST_DATA['quotaAbbr']))).'" OR UCASE(quotaName)="'.add_slashes(strtoupper(trim($REQUEST_DATA['quotaName']))).'" ) ');
        if(trim($foundArray[0]['quotaAbbr'])=='') {  //DUPLICATE CHECK
            $returnStatus = QuotaManager::getInstance()->addQuota();
