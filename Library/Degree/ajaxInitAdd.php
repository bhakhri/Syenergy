<?php
//-------------------------------------------------------
// THIS FILE IS USED TO ADD A Degree
//
//
...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = DegreeManager::getInstance()->getDegree(' WHERE UCASE(degreeCode)="'.add_slashes(strtoupper($REQUEST_DATA['degreeCode'])).'" OR UCASE(degreeName)="'.add_slashes(strtoupper($REQUEST_DATA['degreeName'])).'" OR UCASE(degreeAbbr)="'.add_slashes(strtoupper($REQUEST_DATA['degreeAbbr'])).'"');
        if(trim($foundArray[0]['degreeCode'])=='') {  //DUPLICATE CHECK
            $returnStatus = DegreeManager::getInstance()->addDegree();
