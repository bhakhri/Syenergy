...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = CleaningRoomManager::getInstance()->getCleaningRoom('AND et.tempEmployeeId ="'.add_slashes(trim($REQUEST_DATA['safaiwala'])).'" AND hs.hostelId="'.add_slashes(trim($REQUEST_DATA['hostelName'])).'" AND hcr.Dated ="'.add_slashes(trim($REQUEST_DATA['date'])).'" AND hcr.cleanId!='.$REQUEST_DATA['cleanId']);
			if(trim($foundArray[0]['tempEmployeeId'])=='') { //DUPLICATE CHECK
				$returnStatus = CleaningRoomManager::getInstance()->editCleaningRoom($REQUEST_DATA['cleanId']);
...
// $History: ajaxInitCleaningRoomDetailEdit.php $
//
//*****************  Version 3  *****************
