...
UtilityManager::headerNoCache();
	...
	$foundArrays=DriveManager::getInstance()->getPlacementDrives();
	$c = count($foundArrays);
	for($i=0;$i<$c;$i++) {
	    if(strtoupper(trim($foundArrays[$i]['placementDriveCode']))==strtoupper(trim($REQUEST_DATA['driveCode']))){
    ...
    if(trim($errorMessage) == '') {
      if(SystemDatabaseManager::getInstance()->startTransaction()) {
          $returnStatus = DriveManager::getInstance()->addPlacementDrive();
          if($returnStatus == false) {
           die(FAILURE);
          }
          //get last insert id.
          $placementDriveId=SystemDatabaseManager::getInstance()->lastInsertId();
