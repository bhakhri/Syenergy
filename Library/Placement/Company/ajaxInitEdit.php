...
UtilityManager::headerNoCache();
  ...
  //  }
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = CompanyManager::getInstance()->getCompany(' AND ( UCASE(companyName)="'.add_slashes(trim(strtoupper($REQUEST_DATA['companyName']))).'" OR UCASE(companyCode)="'.add_slashes(trim(strtoupper($REQUEST_DATA['companyCode']))).'" OR UCASE(emailId)="'.add_slashes(trim(strtoupper($REQUEST_DATA['emailId']))).'" ) AND companyId!='.trim($REQUEST_DATA['companyId']));
        if(trim($foundArray[0]['companyName'])=='') {  //DUPLICATE CHECK
            $returnStatus = CompanyManager::getInstance()->editCompany($REQUEST_DATA['companyId']);
