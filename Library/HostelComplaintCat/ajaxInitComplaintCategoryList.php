...
//--------------------------------------------------------
    ...
    $totalArray = $complaintManager->getTotalComplaintCategory($filter);

    $complaintRecordArray = $complaintManager->getComplaintCategoryList('', $filter, $orderBy, $limit);
       
    $cnt = count($complaintRecordArray);
    
    for($i=0;$i<$cnt;$i++) {
        // add stateId in actionId to populate edit/delete icons in User Interface   
        $valueArray = array_merge(array('action' => $complaintRecordArray[$i]['complaintCategoryId'] , 'srNo' => ($records+$i+1) ),$complaintRecordArray[$i]);
