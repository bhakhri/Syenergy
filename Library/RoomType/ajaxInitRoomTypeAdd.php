...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = RoomTypeManager::getInstance()->getRoomType(' WHERE UCASE(roomType)= "'.add_slashes(trim(strtoupper($REQUEST_DATA['roomType']))).'" OR UCASE(abbr)="'.add_slashes(strtoupper($REQUEST_DATA['abbr'])).'"');
                 if(trim($foundArray[0]['abbr'])=='') {  //DUPLICATE CHECK  
                   $returnStatus = RoomTypeManager::getInstance()->addRoomType();
