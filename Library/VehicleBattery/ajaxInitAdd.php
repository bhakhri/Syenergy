...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = VehicleBatteryManager::getInstance()->getVehicleBattery(' AND UCASE(batteryNo)="'.add_slashes(trim(strtoupper($REQUEST_DATA['batteryNo']))).'"');
		if(SystemDatabaseManager::getInstance()->startTransaction()) {
			if(trim($foundArray[0]['batteryNo'])=='') {  //DUPLICATE CHECK
				$returnStatus = VehicleBatteryManager::getInstance()->addVehicleBattery();
