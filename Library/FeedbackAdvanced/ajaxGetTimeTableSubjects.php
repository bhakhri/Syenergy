...
if(trim($REQUEST_DATA['timeTableLabelId'] ) != '' and trim($REQUEST_DATA['classId'] ) != '') {
    ...
    $foundArray=FeedBackTeacherMappingManager::getInstance()->getTimeTableSubjects(' AND ttc.timeTableLabelId="'.trim($REQUEST_DATA['timeTableLabelId']).'" AND c.classId="'.trim($REQUEST_DATA['classId']).'"');
