...
UtilityManager::headerNoCache();
    ...
    if(trim($REQUEST_DATA['employeeId'])!='' and trim($REQUEST_DATA['classId'])!='' and trim($REQUEST_DATA['timeTableLabelId'])!=''){ //for daily attendance
    $startDate=date('Y-m-d');
    $endDate=date('Y-m-d');
    //*************Get adjusted data for class*************
     $foundArray = AdminTasksManager::getInstance()->getDutyLeaveSubjects(trim($REQUEST_DATA['employeeId']),trim($REQUEST_DATA['classId']),trim($REQUEST_DATA['timeTableLabelId']),$startDate,$endDate);
    }
    else{
        echo 0;
        die;
...
// $History: ajaxGetDutyLeaveSubject.php $
//
//*****************  Version 1  *****************
