...
UtilityManager::headerNoCache();
    ...
    if(trim($REQUEST_DATA['employeeId'])!='' and trim($REQUEST_DATA['timeTableLabelId'])!=''){ //for daily attendance
    $startDate=date('Y-m-d');
    $endDate=date('Y-m-d');
    //*************Get adjusted data for class*************
     $foundArray = AdminTasksManager::getInstance()->getDutyLeaveClasses(trim($REQUEST_DATA['employeeId']),trim($REQUEST_DATA['timeTableLabelId']),$startDate,$endDate);
    }
    else{
        echo 0;
        die;
...
// $History: ajaxGetDutyLeaveClass.php $
//
//*****************  Version 1  *****************
