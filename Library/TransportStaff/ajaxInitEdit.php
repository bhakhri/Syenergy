...
UtilityManager::headerNoCache();
  ...
  if (!isset($REQUEST_DATA['staffName']) || trim($REQUEST_DATA['staffName']) == '') {
    ...
    if($REQUEST_DATA['leavingDate'] !='') { 
	    if(strtotime($REQUEST_DATA['join']) > strtotime($REQUEST_DATA['leavingDate']) ){
		    echo LEAVING_DATE_VALIDATION1;
		    die;
	    }
	    
        if(strtotime("now") < strtotime($REQUEST_DATA['leavingDate']) ){
		    echo LEAVING_DATE_VALIDATION2;
		    die;
	    }
    ...
    $foundArray = TransportStaffManager::getInstance()->getTransportStaff(' WHERE (UCASE(staffCode)="'.add_slashes(trim(strtoupper($REQUEST_DATA['staffCode']))).'" OR dlNo = "'.add_slashes(trim(strtoupper($REQUEST_DATA['dlNo']))).'") AND staffId!='.$REQUEST_DATA['staffId']);
	if(SystemDatabaseManager::getInstance()->startTransaction()) {
        if(trim($foundArray[0]['staffCode'])=='') {  //DUPLICATE CHECK
            $returnStatus = TransportStaffManager::getInstance()->editTransportStaff($REQUEST_DATA['staffId']);
		...
		if(SystemDatabaseManager::getInstance()->commitTransaction()) {
			$sessionHandler->setSessionVariable('IdToFileUpload',$REQUEST_DATA['staffId']);
			echo SUCCESS;
			die;
		}
		else {
