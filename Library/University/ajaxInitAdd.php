...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = UniversityManager::getInstance()->getUniversity(' WHERE UCASE(universityCode)="'.add_slashes(trim(strtoupper($REQUEST_DATA['universityCode']))).'" OR UCASE(universityAbbr)="'.add_slashes(trim(strtoupper($REQUEST_DATA['universityAbbr']))).'"');
        if(trim($foundArray[0]['universityCode'])=='') {  //DUPLICATE CHECK
            $returnStatus = UniversityManager::getInstance()->addUniversity();
