<?php
//-------------------------------------------------------
// THIS FILE IS USED TO EDIT EXISTING PERIOD AND USER & CHECK THE DUPLICACY ALSO
//
//
// Author : Jaineesh
...
UtilityManager::headerNoCache();
    ...
    if ($errorMessage == '' && (!isset($REQUEST_DATA['periodNumber']) || trim($REQUEST_DATA['periodNumber']) == '')) {
...
#############	LOGIC FOR TIME BASED CHECKING STARTS :) #########################################
	...
	$periodId = $REQUEST_DATA['periodId'];
	$periodDetailsArray = $periodsManager->getPeriodSlotPeriods($periodSlotId, $periodId);
...
#############	LOGIC FOR TIME BASED CHECKING ENDS :) #########################################
    ...
    if (trim($errorMessage) == '') {
		...
		if ($startTime != '' AND $endTime != '') {
			//$foundTimeArray = PeriodsManager::getInstance()->getPeriods('AND (("'.$periodStartTime.'" BETWEEN pr.startTime AND pr.endTime) AND ("'.$periodStartTime.'" < (pr.endTime)) OR ("'.$periodSecondTime.'" BETWEEN pr.startTime AND pr.endTime) OR ("'.$periodStartTime.'" < pr.startTime AND pr.endTime >= "'.$periodSecondTime.'")) AND pr.periodSlotId = "'.add_slashes(strtoupper($REQUEST_DATA['slotName'])).'" AND pr.periodId !='.$REQUEST_DATA['periodId']);
            $foundTimeArray = PeriodsManager::getInstance()->getPeriods('AND (("'.$periodStartTime.'" BETWEEN pr.startTime AND pr.endTime) AND ("'.$periodStartTime.'" < (pr.endTime)) AND ("'.$periodSecondTime.'" BETWEEN pr.startTime AND pr.endTime)) AND pr.periodSlotId = "'.add_slashes(strtoupper($REQUEST_DATA['slotName'])).'" AND pr.periodId !='.$REQUEST_DATA['periodId']);  
		...
		$foundArray = PeriodsManager::getInstance()->getPeriods(' AND UCASE(pr.periodNumber)="'.add_slashes(trim(strtoupper($REQUEST_DATA['periodNumber']))).'" AND pr.periodSlotId="'.add_slashes(strtoupper($REQUEST_DATA['slotName'])).'" AND pr.periodId !='.$REQUEST_DATA['periodId']);
