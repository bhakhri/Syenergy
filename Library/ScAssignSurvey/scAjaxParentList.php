...
function parseName($value){
  ...
  return $genName;
    ...
    $totalArray = $assignSurveyManager->getTotalParent($conditions);
    $studentRecordArray = $assignSurveyManager->getParentList($conditions,$limit,$orderBy);
    $cnt = count($studentRecordArray);
    
    $selectedParent=explode(",",$REQUEST_DATA['selectedParent']);
    $len=count($selectedParent);  
    
    $onclick='onclick="checkUncheckParent(this.value,this.checked);"';  //adding onclick handler 
    
    for($i=0;$i<$cnt;$i++) {
       if($len>1 and is_array($selectedParent)){  //check for initial values
        if(in_array($studentRecordArray[$i]['studentId'],$selectedParent)){
          $valueArray = array_merge(array('srNo' => ($records+$i+1),"parents" => "<input type=\"checkbox\" name=\"parents\" id=\"parents\" checked=\"checked\" value=\"".$studentRecordArray[$i]['studentId'] ."\" $onclick>")
        , $studentRecordArray[$i]);
        }
        else{
          $valueArray = array_merge(array('srNo' => ($records+$i+1),"parents" => "<input type=\"checkbox\" name=\"parents\" id=\"parents\" value=\"".$studentRecordArray[$i]['studentId'] ."\" $onclick>")
        , $studentRecordArray[$i]);
        }
      }
     else{
       if($studentRecordArray[$i]['parentAssigned']=='Yes'){ 
        $valueArray = array_merge(array('srNo' => ($records+$i+1),"parents" => "<input type=\"checkbox\" name=\"parents\" id=\"parents\" checked=\"checked\" value=\"".$studentRecordArray[$i]['studentId'] ."\" $onclick>")
         , $studentRecordArray[$i]);
       }
       else{
    ...
    echo '{"sortOrderBy":"'.$sortOrderBy.'","sortField":"'.$sortField.'","totalRecords":"'.count($totalArray).'","page":"'.$page.'","info" : ['.$json_val.'],"parentInfo" : '.json_encode($totalArray).'}';  
    
// for VSS
// $History: scAjaxParentList.php $
//
//*****************  Version 3  *****************
