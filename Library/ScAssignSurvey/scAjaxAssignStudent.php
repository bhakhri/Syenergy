...
$searchIdArr=$assignSurveyManager->checkStudentExistence("WHERE svu.targetIds = scs.studentId AND scs.userId = fsa.userId AND svu.userType='S'");
$studentIdList = UtilityManager::makeCSList($searchIdArr, 'studentId');
$cntSearch = count($searchIdArr);

if (trim($errorMessage) == '') {
    $studentIds=explode(",",$REQUEST_DATA['student']);
    $cnt=count($studentIds);
    $surveyId=$REQUEST_DATA['surveyId'];
    $userType='S';
    
	$studentExplodeList = explode(",",$studentIdList);
	$resultArr = array_diff($studentExplodeList,$studentIds);
	
    for($i=0;$i<$cnt;$i++){

        if($insQuery==''){
            $insQuery=" INSERT IGNORE INTO survey_visible_to_users (feedbackSurveyId,targetIds,userType)
                        VALUES($surveyId,$studentIds[$i],'".$userType."')";
        }
        else{
            $insQuery = $insQuery .", ($surveyId,$studentIds[$i],'".$userType."')";
        }
    }
    $ret1=false;
    $ret=true;
   ...
   // $ret1=$assignSurveyManager->deleteAssignedSurvey($surveyId,$userType,"AND studentId NOT IN (".$REQUEST_DATA['student'].") AND studentId not in ($studentIdList)");
   if ($cntSearch>0 && is_array($searchIdArr)) {
    ...
    elseif($ret1===true and $ret===false){
		if($cntSearch < 0 OR $cntSearch == "") {
			echo SURVEY_DEASSINED_STUDENT; //if previous data deleted and new records not inserted
		}
		else {
			echo SUCCESS;
		}
...
// $History: scAjaxAssignStudent.php $
//
//*****************  Version 1  *****************
