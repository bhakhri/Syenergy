...
$sendMessageManager = SendMessageManager::getInstance();
  ...
  if(UtilityManager::notEmpty($sessionHandler->getSessionVariable('IdToFileUpload'))) {
    ...
    $ffName = $sessionHandler->getSessionVariable('IdToFileUpload')."_".$fileObj->name;
    $ffName = str_replace(',','_',$ffName);
    $ffName = str_replace('"','',$ffName);
    $ffName = str_replace("'",'',$ffName);
    $ffName = str_replace("`",'',$ffName);
             
    $extArr = explode('.',$ffName);
    $ext = $extArr[count($extArr)-1];
    $serverDate = date('d-m-Y');
    $serverDate = str_replace('-','_',$serverDate);
    
    $filename = $extArr[0].'_'.$serverDate;
    if(trim($ext)!='') {
      $filename .= ".".trim($ext);
    }
