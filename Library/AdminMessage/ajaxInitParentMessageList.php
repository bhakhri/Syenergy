...
//--------------------------------------------------------
    ...
    $filter .= " AND stc.receiverId = ".$userId;
    
    $totalArray = $adminMessageManager->getTotalParentAdminMessage($filter);


    $parentTeacherRecordArray = $adminMessageManager->getParentTeacherAdminList($filter,$limit,$orderBy);
	

  $receivesms=$parentTeacherRecordArray[0]['visibleToParent'];
    $cnt = count($parentTeacherRecordArray);

     $nonTeacherNonSmsParent=$sessionHandler->getSessionVariable('NONTEACHER_NONSMS_PARENTS'); 


if($roleId!=2){
	if($nonTeacherNonSmsParent==0) {
		$totalCount=0;
		echo '{"sortOrderBy":"'.$sortOrderBy.'","sortField":"'.$sortField.'","totalRecords":"'.$totalCount.'","page":"'.$page.'","info" : ['.$json_val.']}'; 
	} 
		 ...
		 if($receivesms== 1){
				...
				$actionStr1='<a href="#" title="Reply"><img src="'.IMG_HTTP_PATH.'/reply_mail.gif" border="0" alt="Reply" onclick="editWindow('.	 					$parentTeacherRecordArray[$i]['messageId'].',\'ParentTeacherActionDiv\');return false;"></a>';
            			...
            			$actionStr3='<a href="#" title="Delete"><img src="'.IMG_HTTP_PATH.'/delete1.gif" border="0" alt="Delete" 
				onclick="deleteParentTeacher('.$parentTeacherRecordArray[$i]['messageId'].');return false;"></a>';
			...
			echo '{"sortOrderBy":"'.$sortOrderBy.'","sortField":"'.$sortField.'","totalRecords":"'.$totalCount.'","page":"'.$page.'","info" : ['.$json_val.']}'; 
		     ...
		     if($receivesms== 1){
        		...
        		// add stateId in actionId to populate edit/delete icons in User Interface   
			if($parentTeacherRecordArray[$i]['attachmentFile']!=''){
				$parentTeacherRecordArray[$i]['attachmentFile'] ='<img src="'.IMG_HTTP_PATH.'/download.gif" title="'.$parentTeacherRecordArray[$i]	
				['messageSubject'].'" onclick="javascript:download(\''.$parentTeacherRecordArray[$i]['attachmentFile'].'\')" />';  
			}
			else{
				$parentTeacherRecordArray[$i]['attachmentFile'] = NOT_APPLICABLE_STRING;
			}
