<?php
//-------------------------------------------------------
// THIS FILE IS USED TO EDIT A SUPPLIER
//
//
    ...
    $errorMessage ='';
	 ...
	 if (trim($errorMessage) == '') {
        require_once(MODEL_PATH . "/SupplierManager.inc.php");
                 $foundArray = SupplierManager::getInstance()->getSupplier(' AND (UCASE(companyName) = "'.add_slashes(trim(strtoupper($REQUEST_DATA['companyName']))).'" OR UCASE(supplierCode)="'.add_slashes(strtoupper($REQUEST_DATA['supplierCode'])).'") AND supplierId!='.$REQUEST_DATA['supplierId']);
                 if(trim($foundArray[0]['supplierCode'])=='') {  //DUPLICATE CHECK  
                   $returnStatus = SupplierManager::getInstance()->editSupplier($REQUEST_DATA['supplierId']);
