...
//--------------------------------------------------------
    ...
    $stateRecordArray = $stateManager->getStateList($filter,$limit,$orderBy);
    $cnt = count($stateRecordArray);
    
    for($i=0;$i<$cnt;$i++) {
        // add stateId in actionId to populate edit/delete icons in User Interface   
        $valueArray = array_merge(array('action' => $stateRecordArray[$i]['stateId'] , 'srNo' => ($records+$i+1) ),$stateRecordArray[$i]);
