...
//  This File calls Edit Function used in adding "FeeFundAllocation" Records
//
...
$queryDescription =''; 
   ...
   if (!isset($REQUEST_DATA['allocationEntity']) || trim($REQUEST_DATA['allocationEntity']) == '') {
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = FeeFundAllocationManager::getInstance()->getFeeFundAllocation(' AND UCASE(allocationEntity)="'.add_slashes(strtoupper($REQUEST_DATA['allocationEntity'])).'" AND feeFundAllocationId!='.$REQUEST_DATA['feeFundAllocationId']);
        if(trim($foundArray[0]['allocationEntity'])=='') {  //DUPLICATE CHECK
          $foundArray = FeeFundAllocationManager::getInstance()->getFeeFundAllocation(' AND UCASE(entityType)="'.add_slashes(strtoupper($REQUEST_DATA['entityType'])).'" AND feeFundAllocationId!='.$REQUEST_DATA['feeFundAllocationId']);
          if(trim($foundArray[0]['allocationEntity'])=='') {  //DUPLICATE CHECK
            $returnStatus = FeeFundAllocationManager::getInstance()->editFeeFundAllocation($REQUEST_DATA['feeFundAllocationId']);            
			...
			$auditTrialDescription = "Following Fund Allocation Entity Has Been Edited: ";
			$auditTrialDescription .= $REQUEST_DATA['allocationEntity'];
			$type = FUND_ALLOCATION_ENTITY_EDITED; 
...
//
//*****************  Version 4  *****************
//User: Arvind       Date: 8/05/08    Time: 1:36p
//Updated in $/Leap/Source/Library/FeeFundAllocation
//modified
//
//*****************  Version 3  *****************
//User: Arvind       Date: 8/01/08    Time: 6:42p
//Updated in $/Leap/Source/Library/FeeFundAllocation
//added validation for duplicacy
//
//*****************  Version 2  *****************
