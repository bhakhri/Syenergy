...
//--------------------------------------------------------
	...
	$status = $feeLedgerManager->addDebitCredit($classId,$studentId,$particulars,$debit,$credit,$feeCycleId,$ledgerTypeId);
	...
	//Check for Generate Fee Student Table		start		
		 if($sessionHandler->getSessionVariable('STUDENT_GENERATE_FEE')=='1'){
			...
			if($ledgerTypeId=='1'){
				$ledgerAcademicDebit = $debit + $debitAmount;
				$ledgerAcademicCredit = $credit + $creditAmount;
				
			}
			if($ledgerTypeId=='2'){
				$ledgerTransportDebit = $debit + $debitAmount;
				$ledgerTransportCredit = $credit + $creditAmount;
			}
			if($ledgerTypeId=='3'){
				$ledgerHostelDebit = $debit + $debitAmount;
				$ledgerHostelCredit = $credit + $creditAmount;
			}
