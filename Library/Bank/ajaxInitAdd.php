...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = BankManager::getInstance()->getBankName(' WHERE UCASE(bankName)="'.add_slashes(trim(strtoupper($REQUEST_DATA['bankName']))).'"');  

        if(trim($foundArray[0]['bankName'])=='') {  //DUPLICATE CHECK
			...
			if (trim($foundArray2[0]['bankAbbr'] == '')) {
				$returnStatus = BankManager::getInstance()->addBank();
				...
				if($returnStatus === false) {
					...
					$auditTrialDescription = "Following Bank Name Has Been Created ";
					$auditTrialDescription .= $REQUEST_DATA['bankName'];
					$type = BANK_NAME_IS_CREATED; 
