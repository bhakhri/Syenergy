<?php 
//
//  This File calls Edit Function used in adding Branch Records
//
...
UtilityManager::headerNoCache();  //print_r($REQUEST_DATA);
    $errorMessage ='';
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = BranchManager::getInstance()->getBranch(' WHERE UCASE(branchName)="'.add_slashes(strtoupper($REQUEST_DATA['branchName'])).'" AND branchId!='.$REQUEST_DATA['branchId']);
        if(trim($foundArray[0]['branchCode'])=='') {  //DUPLICATE CHECK
          $foundArray = BranchManager::getInstance()->getBranch(' WHERE UCASE(branchCode)="'.add_slashes(strtoupper($REQUEST_DATA['branchCode'])).'" AND branchId!='.$REQUEST_DATA['branchId']);
            if(trim($foundArray[0]['branchCode'])=='') {  //DUPLICATE CHECK
                $returnStatus = BranchManager::getInstance()->editBranch($REQUEST_DATA['branchId']);
