...
UtilityManager::headerNoCache();
	...
	if (trim($errorMessage) == '') {
        ...
        $foundArray = ClassManager::getInstance()->getClass(' WHERE UCASE(className)="'.add_slashes(strtoupper($REQUEST_DATA['className'])).'"');
        if(trim($foundArray[0]['className'])=='') {  //DUPLICATE CHECK
            $returnStatus = ClassManager::getInstance()->addClass();
