...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
		...
		$foundArray = DesignationTempManager::getInstance()->getDesignation(' WHERE LCASE(designationName)= "'.add_slashes(trim(strtolower($REQUEST_DATA['designationName']))).'"');
		if(trim($foundArray[0]['designationName'])=='') {  //DUPLICATE CHECK
			$foundArray2 = DesignationTempManager::getInstance()->getDesignation(' WHERE UCASE(designationCode)="'.add_slashes(strtoupper($REQUEST_DATA['designationCode'])).'"');
			if(trim($foundArray2[0]['designationCode'])=='') {  //DUPLICATE CHECK
				$returnStatus = DesignationTempManager::getInstance()->addDesignation();
