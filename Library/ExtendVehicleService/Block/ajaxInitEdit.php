<?php
//-------------------------------------------------------
// Purpose: To update block table data
//
...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = BlockManager::getInstance()->getBlock(' WHERE ( ( UCASE(blockName)="'.add_slashes(strtoupper(trim($REQUEST_DATA['blockName']))).'" OR UCASE(abbreviation)="'.add_slashes(strtoupper(trim($REQUEST_DATA['abbreviation']))).'" ) AND buildingId='.$REQUEST_DATA['buildingId'].') AND blockId!='.$REQUEST_DATA['blockId']);
        if(trim($foundArray[0]['blockName'])=='') {  //DUPLICATE CHECK
            $returnStatus = BlockManager::getInstance()->editBlock($REQUEST_DATA['blockId']);
