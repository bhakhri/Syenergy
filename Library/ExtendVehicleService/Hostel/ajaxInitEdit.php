...
UtilityManager::headerNoCache();
    ...
    if ($errorMessage == '' && (!isset($REQUEST_DATA['wardenName']) || trim($REQUEST_DATA['wardenName']) == '')) {
        $errorMessage .= "Enter Warden Name \n";
    }
    if ($errorMessage == '' && (!isset($REQUEST_DATA['wardenContactNo']) || trim($REQUEST_DATA['wardenContactNo']) == '')) {
        $errorMessage .= "Enter Warden Contact No. \n";  
    }
    if (trim($errorMessage) == '') {
        require_once(MODEL_PATH . "/HostelManager.inc.php");
		$foundArray = HostelManager::getInstance()->getHostel(' WHERE (UCASE(hostelName)= "'.add_slashes(trim(strtoupper($REQUEST_DATA['hostelName']))).'" OR UCASE(hostelCode)="'.add_slashes(strtoupper($REQUEST_DATA['hostelCode'])).'") AND hostelId!='.$REQUEST_DATA['hostelId']);

                 if(trim($foundArray[0]['hostelCode'])=='') {  //DUPLICATE CHECK  
                   $returnStatus = HostelManager::getInstance()->editHostel($REQUEST_DATA['hostelId']);
