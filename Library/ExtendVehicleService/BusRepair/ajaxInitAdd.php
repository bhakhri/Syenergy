...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = BusRepairManager::getInstance()->getBusRepair(' WHERE ( UCASE(billNumber)="'.add_slashes(trim(strtoupper($REQUEST_DATA['billNumber']))).'" AND UCASE(workshopName)="'.add_slashes(trim(strtoupper($REQUEST_DATA['workShop']))).'" )');
        if(trim($foundArray[0]['billNumber'])=='') {  //DUPLICATE CHECK
            $returnStatus = BusRepairManager::getInstance()->addBusRepair();
            if($returnStatus === false) {
                ...
                if(trim($REQUEST_DATA['actionIds'])!=''){
                 ...
                 $lastRepairId=SystemDatabaseManager::getInstance()->lastInsertId();
                 ...
                 for($k=0;$k<$len;$k++){
                     ...
                     $insStr .=" ($lastRepairId,$actionIds[$k],'".$dueDates[$k]."' ) ";
                 }
                 if($insStr!=''){
                   $ret2=BusRepairManager::getInstance()->addActionRepaired($lastRepairId,$insStr);
                   if($ret2===false){
                       echo FAILURE;
                       die;
                   }
