...
$doneByUserId=$sessionHandler->getSessionVariable('UserId');  
    ...
    //This loop will block the pending students 
       $unBlockStudent='';
       $userIds= array();
       $j=0;
       $strQuery='';
       for($i=0;$i<$cnt;$i++) {
         ...
         $isPendingStatus =2;
         $isBlockStatus = 0;  // 0 => Unblock Student, 1=> Block Student, 2=> Unblock By Admin
   ...
   if(SystemDatabaseManager::getInstance()->startTransaction()) {
        if($strQuery!='') {
         ...
         if($unBlockStudent!='') {
             ...
             $returnStatus=$fbManager->changeStudentStatusInBlock1($condition,1);
             if($returnStatus === false) {
               echo FAILURE;
                die;
             }  
