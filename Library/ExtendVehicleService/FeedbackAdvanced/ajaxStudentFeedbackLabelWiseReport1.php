...
function parseName($value){
  ...
  if($type==1) {  
    ...
    $totalRecordArray=$fbManager->getStudentsFeedbackStatus($conditions,$labelId,$orderBy,'',$timeTableLabelId,$typeOf);  
    $studentRecordArray=$fbManager->getStudentsFeedbackStatus($conditions,$labelId,$orderBy,$limit,$timeTableLabelId,$typeOf);  
    ...
    for($j=0;$j<count($studentRecordArray);$j++) {
       $actionString='';
       $hidden = '<input type="hidden" id="totalStudentHiddenId'.$j.'" name="totalStudentHiddenId[]" value="'.$studentRecordArray[$j]['studentId'].'">';
       if($type==1){ //for students
          if(strtolower(trim($studentRecordArray[$j]['isCompleted'])) == 'completed') { 
             $studentRecordArray[$j]['currentStatus']='Done';
             $actionString=NOT_APPLICABLE_STRING;
          }
          else { 
              if($studentRecordArray[$j]['dateOverFlag']==1){
                 $isCompleted=strtolower(trim($studentRecordArray[$j]['isCompleted'])); 
                 $dateOver = strtolower(trim($studentRecordArray[$j]['dateOver']));
                 $currentStatus = strtolower(trim($studentRecordArray[$j]['currentStatus'])); 
                 $msg='';                                                                     
                 // CurrentStatus: 0 => Unblock Student, 1=> Block Student, 2=> Unblock By Admin      
                 if($isCompleted == 'partial' || $isCompleted=='pending') {    
                    if($currentStatus==1){
                       $studentRecordArray[$j]['currentStatus']='Blocked';
                       $actionString=$hidden.'<a href="javascript:void(0);" title="Unblock this student"><img style="height:20px;" src="'.IMG_HTTP_PATH.'/selected.gif" onclick="changeStatus('.$studentRecordArray[$j]['studentId'].',2)" ></a>';
                    }
                    else if($currentStatus==0){
                       if($isCompleted == 'partial' ) {
                         $msg ='In Progress';       //Partial messgae check
                       }
                       else {
                         $msg = 'Not Attempted';     //Pending messgae check  
                       }
                       if($dateOver=='yes'){
                         $msg.='<br>(Date Over)';       
                       }
                       $studentRecordArray[$j]['currentStatus']="<b>$msg</b>";
                       $actionString=$hidden.'<a href="javascript:void(0);" title="Block this student"><img style="height:20px;" src="'.IMG_HTTP_PATH.'/block_user.png" onclick="changeStatus('.$studentRecordArray[$j]['studentId'].',1);" ></a>';
                   }
                   else {
          ...
          $valueArray = array_merge(array('srNo' => ($records+$j+1),'actionString'=>$actionString ),$studentRecordArray[$j]); 
          if(trim($json_val)=='') {
  ...
  //***********EMPLOYEE SEARCH FILTER ENDS************
   ...
   // $conditions .='  AND fss.timeTableLabelId='.$timeTableLabelId;
