...
function parseName($value){
  ...
  return $genName;
    ...
    for($i=0;$i<$cnt;$i++) {
       if($studentRecordArray[$i]['dateOverFlag']==1){
         if($studentRecordArray[$i]['isCompletedFlag']!=1){
            $blockStudentArray[]=$studentRecordArray[$i]['userId'];
         }
       }
    }
   
   $cnt=count($blockStudentArray);
   if($cnt==0){
       die("No students found for blocking");
   }
   ...
   if(SystemDatabaseManager::getInstance()->startTransaction()) {
      ...
      $ret=$fbManager->changeStudentStatusInBlock(implode(',',$blockStudentArray),FEEDBACK_STUDENT_BLOCKED);
      if($ret==false){
       echo FAILURE;
       die;
      } 
