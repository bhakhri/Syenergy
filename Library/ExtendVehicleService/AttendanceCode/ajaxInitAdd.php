<?php 
//  This File calls addFunction used in adding AttendanceCode Records
//
...
UtilityManager::headerNoCache();     
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = AttendanceCodeManager::getInstance()->getAttendanceCode(' WHERE UCASE(attendanceCodeName)="'.add_slashes(strtoupper($REQUEST_DATA['attendanceCodeName'])).'"');
        if(trim($foundArray[0]['attendanceCodeName'])=='') {  //DUPLICATE CHECK
           $foundArray = AttendanceCodeManager::getInstance()->getAttendanceCode(' WHERE UCASE(attendanceCode)="'.add_slashes(strtoupper($REQUEST_DATA['attendanceCode'])).'"');
            if(trim($foundArray[0]['attendanceCode'])=='') {  //DUPLICATE CHECK
                $returnStatus = AttendanceCodeManager::getInstance()->addAttendanceCode();
...
//Updated in $/Leap/Source/Library/AttendanceCode
//removed validation for attendanceCodeAction
//
//*****************  Version 2  *****************
