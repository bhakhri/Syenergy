...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = CountryManager::getInstance()->getCountry(' WHERE UCASE(countryName)="'.add_slashes(strtoupper($REQUEST_DATA['countryName'])).'"');
        if(trim($foundArray[0]['countryCode'])=='') {  //DUPLICATE CHECK
           $foundArray = CountryManager::getInstance()->getCountry(' WHERE UCASE(countryCode)="'.add_slashes(strtoupper($REQUEST_DATA['countryCode'])).'"');
            if(trim($foundArray[0]['countryCode'])=='') {  //DUPLICATE CHECK
               $foundArray = CountryManager::getInstance()->getCountry(' WHERE UCASE(nationalityName)="'.add_slashes(strtoupper($REQUEST_DATA['nationalityName'])).'"');
                if(trim($foundArray[0]['countryCode'])=='') {  //DUPLICATE CHECK
			        $returnStatus = CountryManager::getInstance()->addCountry();
...
//
//*****************  Version 5  *****************
//User: Arvind       Date: 7/16/08    Time: 6:46p
//Updated in $/Leap/Source/Library/Country
//added a check for countryName in a query
//
//*****************  Version 4  *****************
