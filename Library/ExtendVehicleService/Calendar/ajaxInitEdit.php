<?php
//-------------------------------------------------------
// THIS FILE IS USED TO EDIT An event
//
//
...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = CalendarManager::getInstance()->getEvent(' AND  UCASE(eventTitle)="'.add_slashes(strtoupper($REQUEST_DATA['eventTitle'])).'" AND startDate="'.add_slashes($REQUEST_DATA['startDate']).'" AND eventId!='.$REQUEST_DATA['eventId']);
        if(trim($foundArray[0]['eventTitle'])=='') {  //DUPLICATE CHECK
            $returnStatus = CalendarManager::getInstance()->editEvent($REQUEST_DATA['eventId']);
