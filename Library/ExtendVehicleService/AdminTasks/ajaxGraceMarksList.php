...
//-------------------------------------------------------------------------------------------------------------
    ...
    $conditions ='';
    $conditions = " AND ttm.classId=".$REQUEST_DATA['class1']." AND ttm.subjectId=".$REQUEST_DATA['subject']." AND sg.groupId=".$REQUEST_DATA['group'];
    
    if(trim($REQUEST_DATA['studentRollNo'])!=''){
        $conditions .=' AND s.rollNo LIKE "'.add_slashes(trim($REQUEST_DATA['studentRollNo'])).'%"';
    }
    $graceMarksRecordArray = $teacherManager->getGraceMarksList($conditions,$orderBy);
