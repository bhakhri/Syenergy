...
//-------------------------------------------------------------------------------------------------------------
    ...
    $appraisalManager = AppraisalDataManager::getInstance();
    ...
    $superiorEmpId=trim($sessionHandler->getSessionVariable('EmployeeId'));
    if($superiorEmpId==''){
  ...
  /*DOL--End*/
    ...
    //$totalArray = $appraisalManager->getTotalEmployee($conditions);
	$totalArray = $appraisalManager->getEmployeeList($superiorEmpId,$conditions,'',$orderBy);
	$count = count($totalArray);
    $employeeRecordArray = $appraisalManager->getEmployeeList($superiorEmpId,$conditions,$limit,$orderBy);
    $cnt = count($employeeRecordArray);
    
    for($i=0;$i<$cnt;$i++) {
       $checked=''; 
       $actionStr='<a href="#" title="Appraise Employee"><img src="'.IMG_HTTP_PATH.'/appraisal.jpg" border="0" alt="Appraise Employee" onclick="openAppraisal('.$employeeRecordArray[$i]['employeeId'].');return false;"></a>';
       
       $valueArray = array_merge(array('srNo' => ($records+$i+1),'actionString'=>$actionStr,"emps" => "<input type=\"checkbox\" name=\"emps\" id=\"emps\" value=\"".$employeeRecordArray[$i]['employeeId'] ."\" $checked >"), $employeeRecordArray[$i]);
       if(trim($json_val)=='') {
