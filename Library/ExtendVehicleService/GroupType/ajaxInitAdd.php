<?php
//-------------------------------------------------------
// THIS FILE IS USED TO ADD NEW PERIOD TYPE
//
//
// Author : Jaineesh
...
UtilityManager::headerNoCache();
    ...
    if ($errorMessage == '' && (!isset($REQUEST_DATA['groupTypeName']) || trim($REQUEST_DATA['groupTypeName']) == '')) {
        $errorMessage .=  ENTER_GROUPTYPE_NAME. "<br/>";
    }
    if ($errorMessage == '' && (!isset($REQUEST_DATA['groupTypeCode']) || trim($REQUEST_DATA['groupTypeCode']) == '')) {
        $errorMessage .= ENTER_GROUPTYPE_CODE. "<br/>";
    }
    
    if (trim($errorMessage) == '') {
        require_once(MODEL_PATH . "/GroupTypeManager.inc.php");
		$foundArray = GroupTypeManager::getInstance()->getGroupType(' WHERE LCASE(groupTypeName)= "'.add_slashes(trim(strtolower($REQUEST_DATA['groupTypeName']))).'"');
			if(trim($foundArray[0]['groupTypeName'])=='') {  //DUPLICATE CHECK
				$foundArray2 = GroupTypeManager::getInstance()->getGroupType(' WHERE UCASE(groupTypeCode)="'.add_slashes(strtoupper($REQUEST_DATA['groupTypeCode'])).'"');
