...
UtilityManager::headerNoCache();
	...
	if ($errorMessage == '' && (!isset($REQUEST_DATA['fineType']) || trim($REQUEST_DATA['fineType']) == '')) {
        $errorMessage .= "Select Fine Type"."\n"; 
    }
	
    if (trim($errorMessage) == '') {
        require_once(MODEL_PATH . "/FineManager.inc.php");
        $foundArray = FineManager::getInstance()->getFineCategory(' WHERE UCASE(fineCategoryName)="'.add_slashes(strtoupper(trim($REQUEST_DATA['categoryName']))).'" OR UCASE(fineCategoryAbbr)="'.add_slashes(strtoupper(trim($REQUEST_DATA['categoryAbbr']))).'"');
        if(trim($foundArray[0]['fineCategoryName'])=='') {  //DUPLICATE CHECK
            $returnStatus = FineManager::getInstance()->addFineCategory();
