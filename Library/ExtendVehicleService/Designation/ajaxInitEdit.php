...
UtilityManager::headerNoCache();
	...
	if (trim($errorMessage) == '') {
        require_once(MODEL_PATH . "/DesignationManager.inc.php");
			$foundArray = DesignationManager::getInstance()->getDesignation(' WHERE LCASE(designationName)=  "'.add_slashes(trim(strtolower($REQUEST_DATA['designationName']))).'" AND designationId!='.$REQUEST_DATA['designationId']);
				if(trim($foundArray[0]['designationName'])=='') {  //DUPLICATE CHECK
					$foundArray2 = DesignationManager::getInstance()->getDesignation(' WHERE UCASE(designationCode)="'.add_slashes(strtoupper($REQUEST_DATA['designationCode'])).'" AND designationId!='.$REQUEST_DATA['designationId']);
