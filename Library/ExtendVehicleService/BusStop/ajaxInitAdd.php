...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = BusStopManager::getInstance()->getBusStop(' WHERE ( UCASE(stopAbbr)="'.add_slashes(strtoupper($REQUEST_DATA['stopAbbr'])).'" AND busRouteId='.$REQUEST_DATA['routeCode'].')');
        if(trim($foundArray[0]['stopAbbr'])=='') {  //DUPLICATE CHECK
            $returnStatus = BusStopManager::getInstance()->addBusStop();
