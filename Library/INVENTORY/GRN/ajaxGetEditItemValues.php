...
if(trim($REQUEST_DATA['itemCategoryId'] ) != '' && trim($REQUEST_DATA['poId'] ) != '') {
    require_once(INVENTORY_MODEL_PATH . "/GRNManager.inc.php");
    $grnManager = GRNManager::getInstance();
    
	$itemArray = $grnManager->getEditItem(' AND igt.itemCategoryId='.$REQUEST_DATA['itemCategoryId'].' AND igt.poId='.$REQUEST_DATA['poId']);
