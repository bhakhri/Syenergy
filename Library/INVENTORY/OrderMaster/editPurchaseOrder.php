...
$orderManager = OrderManager::getInstance();
  ...
  if(SystemDatabaseManager::getInstance()->startTransaction()) {
   ...
   //edit the order
   $r=$orderManager->editOrder($orderId,trim($REQUEST_DATA['orderDate']),$REQUEST_DATA['dispatched'],$REQUEST_DATA['dispatchDate']);
   if($r===false){
     echo FAILURE;
     die;       
   }
   
   //first delete the details corresponding to this orderId
   $r1=$orderManager->deleteOrderDetails($orderId);
   if($r1===false){
        echo FAILURE;
        die;  
   }
   
   //then re-add the order details
   $insStr='';
