...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = ItemCategoryManager::getInstance()->getItemCategory(' WHERE UCASE(categoryName)= "'.add_slashes(trim(strtoupper($REQUEST_DATA['categoryName']))).'" OR UCASE(categoryCode)="'.add_slashes(strtoupper($REQUEST_DATA['categoryCode'])).'"');
                 if(trim($foundArray[0]['categoryCode'])=='') {  //DUPLICATE CHECK  
                   $returnStatus = ItemCategoryManager::getInstance()->addItemCategory();
