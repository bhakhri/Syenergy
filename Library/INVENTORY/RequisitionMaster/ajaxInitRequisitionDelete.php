...
UtilityManager::headerNoCache();
    if (!isset($REQUEST_DATA['requisitionId']) || trim($REQUEST_DATA['requisitionId']) == '') {
        $errorMessage = INVALID_REQUISITION;
    }
    ...
    if (trim($errorMessage) == '') {
		...
		$getRequisitionArray = $requisitionManager->getInventoryRequisition("WHERE requisitionId = ".$REQUEST_DATA['requisitionId']." AND requisitionStatus = 1");
		$requisitionId = $getRequisitionArray[0]['requisitionId'];
		if($requisitionId != '') {
			$cancelledRequisitionTransStatus = $requisitionManager->cancelRequisitionTrans($requisitionId);
			if($cancelledRequisitionTransStatus == false) {
				echo FAILURE;
				die;
			}
		}
		else {
			echo ONLY_PENDING_STATUS_CANCELLED;
			die;
		}
