...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
		...
		$foundArray = $requisitionManager->getRequisitionEditData("WHERE LCASE(requisitionNo)= '".add_slashes(trim(strtolower($REQUEST_DATA['requisitionNo'])))."'");
		$requisitionId = $foundArray[0]['requisitionId'];

			if($requisitionId !='') {  //DUPLICATE CHECK

				$deleteTransStatus = $requisitionManager->deleteRequisitionTrans($requisitionId);
