<?php
//-------------------------------------------------------
// THIS FILE IS USED TO Edit An Order
...
$indentId=add_slashes(trim($REQUEST_DATA['indentId']));
   ...
   $indentManager = IndentManager::getInstance();
    ...
    $recordArray=$indentManager->checkIssueedIndent($indentId);
  ...
  //check whether the requested by is an employee or not
  ...
  if(SystemDatabaseManager::getInstance()->startTransaction()) {
   ...
   //edit the indent
   $r=$indentManager->editIndent($indentId,$indentDate,$requestedToUserId,add_slashes(trim($REQUEST_DATA['comments'])));
   if($r===false){
     echo FAILURE;
     die;       
   }
   
   //first delete the details corresponding to this indentId
   $r1=$indentManager->deleteIndentDetails($indentId);
   if($r1===false){
        echo FAILURE;
        die;  
   }
   
   //then re-add the indent details
   $insStr='';
