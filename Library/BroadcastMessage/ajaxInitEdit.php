...
UtilityManager::headerNoCache();
    $errorMessage ='';
    $msgId=trim($REQUEST_DATA['msgId']);
    if($msgId==''){
       die(MESSAGE_NOT_EXIST); 
    }
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = BroadcastMessageManager::getInstance()->getMessage(' WHERE messageDate="'.trim($REQUEST_DATA['msgDate']).'" AND messageId!='.$msgId);
        if(trim($foundArray[0]['messageText'])=='') {  //DUPLICATE CHECK
            $returnStatus = BroadcastMessageManager::getInstance()->editMessage($msgId);
