...
//--------------------------------------------------------
    ...
    $groupArray = $extraClassAttendanceManager->getEmployeeTimeTable($fieldName,$condition,$orderBy); 
    ...
    $fieldName = " DISTINCT p.periodId,  CONCAT(p.periodNumber,' (',p.startTime,p.startAmPm,' to ',p.endTime,p.endAmPm,')') AS periodTime";       
    $orderBy = " ORDER BY p.periodId";
    $periodArray = $extraClassAttendanceManager->getEmployeeTimeTable($fieldName,$condition,$orderBy); 
    
    
    echo json_encode($employeeArray).'~~'.json_encode($classArray).'~~'.json_encode($subjectArray).'~~'.json_encode($groupArray).'~~'.json_encode($periodArray);
