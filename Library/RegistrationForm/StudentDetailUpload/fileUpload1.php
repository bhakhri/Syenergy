    ...
    if(SystemDatabaseManager::getInstance()->startTransaction()) {    
            foreach($sheetNameArray as $sheetIndex=>$value) {
                ...
                for ($i = 2; $i <= $data->sheets[$sheetIndex]['numRows']; $i++) {
                    ...
                    $emailAddress = str_replace("-","_",$emailAddress);     
               ...
               if ($nationality != '') {
                 ...
                 $nationalityId = $nationalityArray[0]['countryId'];  
                    ...
                    if(strtolower($isLeet) == 'yes'){
                     ...
                     /*
                     if($j==16 && $str !='0000.00.00'){
                        if($str != '') {
                           $cnt = substr_count($str, '.');
                           if($cnt == 0 ) {
                             $str = $fontStart1.$str.$fontEnd1;
                              $isError='1'; 
                           }
                           $dateOfBirthDate = explode('.',$str);
                           $dateOfBirthYear = $str[0];
                           $dateOfBirthMonth = $str[1];
                           $dateOfBirthDay = $str[2];
