...
function checkValues($str,$val,$condition){
   ...
   return -1; 
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = FuelManager::getInstance()->getFuel(' AND ( fuel.busId="'.trim($REQUEST_DATA['busId']).'" AND fuel.dated <= "'.$REQUEST_DATA['dated'].'" ) AND fuelId!='.$REQUEST_DATA['fuelId'].' ORDER BY currentMilege ASC');
        ...
        $foundArray2 = FuelManager::getInstance()->getFuel(' AND ( fuel.busId="'.trim($REQUEST_DATA['busId']).'" AND fuel.dated > "'.$REQUEST_DATA['dated'].'" ) AND fuelId!='.$REQUEST_DATA['fuelId'].'  ORDER BY currentMilege ASC');
        if(trim($foundArray2[0]['currentMilege'])!=0 OR trim($foundArray2[0]['currentMilege'])!=''){
         ...
         $returnStatus = FuelManager::getInstance()->editFuel($REQUEST_DATA['fuelId']);
