...
//--------------------------------------------------------
   ...
   */ 
    ...
    if(trim($sessionHandler->getSessionVariable('rStudentId'))!=''){
        $totalArray               = $teacherManager->getTotalStudentGroups($sessionHandler->getSessionVariable('rStudentId'),$REQUEST_DATA['rClassId']);
        $studentGroupsRecordArray = $teacherManager->getStudentGroups($sessionHandler->getSessionVariable('rStudentId'),$REQUEST_DATA['rClassId'],$orderBy,$limit);
    }
    $cnt = count($studentGroupsRecordArray);
    
    for($i=0;$i<$cnt;$i++) {
        // add stateId in actionId to populate edit/delete icons in User Interface   
        $valueArray = array_merge(array('srNo' => ($records+$i+1) ),$studentGroupsRecordArray[$i]);
