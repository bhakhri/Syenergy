...
//--------------------------------------------------------
   ...
   */ 
    ...
    $totalArray      = $teacherManager->getTotalStudentSection($sessionHandler->getSessionVariable('rStudentId'),$REQUEST_DATA['rClassId']);
    $studentCourseRecordArray = $teacherManager->getStudentSection($sessionHandler->getSessionVariable('rStudentId'),$REQUEST_DATA['rClassId'],$orderBy,$limit);
    $cnt = count($studentCourseRecordArray);
    
    for($i=0;$i<$cnt;$i++) {
        // add stateId in actionId to populate edit/delete icons in User Interface   
        $valueArray = array_merge(array('srNo' => ($records+$i+1) ),$studentCourseRecordArray[$i]);
