...
if(trim($REQUEST_DATA['serviceRepairId'] ) != '') {
	...
	for($i=0;$i<$repairCount;$i++) {
		...
		$repairServiceDiv .= '<td width="15%" readonly="true" name="item" id="item" align="left" value="'.$key[$i].'">'.$busRepairTypeArr[$j].'</td>';

		$repairServiceDiv .= '<td width="15%" align="left"><input type="text" readonly="true" class="inputbox1" maxlength="6" name="amount_'.$key[$i].'" id="amount_'.$key[$i].'" value="'.$newArray[$srNoCounter]['amount'].'"></td>';
		$repairServiceDiv .= '<td width="15%" align="left"><input type="text" readonly="true" class="inputbox1" maxlength="9" name="kmRun_'.$key[$i].'" id="kmRun_'.$key[$i].'" value="'.$newArray[$srNoCounter]['kmRun'].'"></td>';
		$repairServiceDiv .= '<td width="15%" align="left"><input type="text" readonly="true" class="inputbox1" maxlength="9" name="kmChangeRun_'.$key[$i].'" id="kmRun_'.$key[$i].'" value = "'.$newArray[$srNoCounter]['changeKM'].'"></td>';
		//}
		$j++;
	}
	...
	//print_r($vehicleServiceRepairDetailArr);
	$count = count($vehicleServiceRepairDetailArr);
	if($count != 0){
		$vehicleServiceRepairDetail .= '<b>Repair Details :</b><p>';
		$vehicleServiceRepairDetail .= '<table border = "1" cellspacing="2" width="100%" style="border:1px solid #cccccc;border-collapse:collapse;" colspan="2">';
		$vehicleServiceRepairDetail .= '<tr class="rowheading">
					<td align="left" width="5%"><b>Sr.No.</b></td><td align="left" width="15%"><b>Type</b></td><td width="10%"><b>Items</b></td><td width="15%"><b>Charges</b></td>
				  </tr>';
		for($i=0;$i<$count;$i++){
			$srNo = $i+1;
			$bg = $bg == "row0" ? "row1" : "row0";
			$vehicleServiceRepairDetail .='<tr class="$bg"><td align="left">'.$srNo.'</td>';
			$vehicleServiceRepairDetail .='<td width="15%" align="left"><input type="text" readonly="true" class="inputbox1" maxlength="6" name="amount_'.$key[$i].'" id="amount_'.$key[$i].'" value="'.$repairTypeResults[$vehicleServiceRepairDetailArr[$i]['type']].'"></td>';
			$vehicleServiceRepairDetail .='<td width="15%" align="left"><input type="text" readonly="true" class="inputbox1" maxlength="6" name="amount_'.$key[$i].'" id="amount_'.$key[$i].'" value="'.$vehicleServiceRepairDetailArr[$i]['item'].'"></td>';
			$vehicleServiceRepairDetail .='<td width="15%" align="left"><input type="text" readonly="true" class="inputbox1" maxlength="6" name="amount_'.$key[$i].'" id="amount_'.$key[$i].'" value="'.$vehicleServiceRepairDetailArr[$i]['amount'].'"></td>';
		}
	}
	...
	*/
	$mainArray['vehicleServiceRepairDetail'] = $vehicleServiceRepairDetail;
