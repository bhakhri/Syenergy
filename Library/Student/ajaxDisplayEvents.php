...
//--------------------------------------------------------
	...
	if(UtilityManager::notEmpty($REQUEST_DATA['searchbox'])) {
		$filter = ' AND (e.eventTitle LIKE "%'.add_slashes($REQUEST_DATA['searchbox']).'%" OR e.shortDescription LIKE "%'.add_slashes($REQUEST_DATA['searchbox']).'%")';
    }
    $sortOrderBy = (UtilityManager::notEmpty($REQUEST_DATA['sortOrderBy'])) ? $REQUEST_DATA['sortOrderBy'] : 'ASC';
    $sortField = (UtilityManager::notEmpty($REQUEST_DATA['sortField'])) ? $REQUEST_DATA['sortField'] : 'e.startDate';
    
    $orderBy = " $sortField $sortOrderBy";

    ////////////
    
    $totalEventsArray = $studentInformationManager->getTotalEvents($filter);
	$instituteRecordArray = $studentInformationManager->getEventList($filter,$orderBy,$limit);
    ...
    for($i=0;$i<$cnt;$i++) {
       ...
       $instituteRecordArray[$i]['startDate']=UtilityManager::formatDate($instituteRecordArray[$i]['startDate']); 
       $instituteRecordArray[$i]['endDate']=UtilityManager::formatDate($instituteRecordArray[$i]['endDate']);
        // add countryId in actionId to populate edit/delete icons in User Interface   
        $valueArray = array_merge(array('instituteEvents'=>$instituteEvent,'sd'=>$shortDescriptions,'Action'=>'<a href="#" title="View Details"><img src="'.IMG_HTTP_PATH.'/zoom.gif"  border="0" width="15" onClick="editWindow('.$instituteRecordArray[$i]['eventId'].',\'ViewEvents\',600,600); return false;"/></a>','action' => $instituteRecordArray[$i]['eventId'], 'srNo' => ($records+$i+1) ),$instituteRecordArray[$i]);
