...
//--------------------------------------------------------
	...
	$adminRecordArray = $studentInformationManager->getAdminMessages($filter,$orderBy,$limit);         
    ...
    for($i=0;$i<$cnt;$i++) {
		...
		$adminRecordArray[$i]['message'] = trim_output(strip_slashes(strip_tags(HtmlFunctions::getInstance()->removePHPJS($adminRecordArray[$i]['message']))),25);
