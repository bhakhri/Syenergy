...
//	$course = $REQUEST_DATA['courseId'];
	...
	if ($roleCount > 0) {
		$totalArray = $studentManager->getTotalRoleStudentFast($conditions,$userId,$showAlumnniStudent);
		$studentRecordArray = $studentManager->getRoleStudentListFast($conditions,$limit,$orderBy,$userId,$showAlumnniStudent);
	}
	else {
    ...
    $sessionHandler->setSessionVariable('IdToStudentList',$conditions);
    ...
    for($i=0;$i<$cnt;$i++) {
		...
		$studentDataPost="studentDetail.php?id=".$studentRecordArray[$i]['studentId'].$qryString."&classId=".$studentRecordArray[$i]['class_id']."&page=".$page."&sortField=".$sortField."&sortOrderBy=".$sortOrderBy."&alumniStudent=".$showAlumnniStudent."" ;
		$studentData='"'.$studentDataPost.'"';
		
		$showlink = "<a href='#' onclick='hideUrlData(".$studentData.",false);' alt='Detail' title='Detail'><img src='".IMG_HTTP_PATH."/zoom.gif' border='0' /></a>&nbsp;&nbsp;<a href='#' onClick='printStudentReport(".$studentRecordArray[$i]['studentId'].",".$studentRecordArray[$i]['class_id'].")' title='Print'><img src='".IMG_HTTP_PATH."/print1.gif' border='0' /></a>";
	$srNo = ($records+$i+1);
	if($studentRecordArray[$i]['studentStatus']==0){
   	   $studentRecordArray[$i]['firstName']= $span1.$studentRecordArray[$i]['firstName'].$span2 ;
	   ...
	   $studentRecordArray[$i]['feeClassName']= $span1.$studentRecordArray[$i]['feeClassName'].$span2 ;
	   $studentRecordArray[$i]['studentMobileNo']= $span1.$studentRecordArray[$i]['studentMobileNo'].$span2 ;
	   $studentRecordArray[$i]['className']= $span1.$studentRecordArray[$i]['className'].$span2 ;
	   $studentRecordArray[$i]['groupId']= $span1.$studentRecordArray[$i]['groupId'].$span2 ;
	   $studentRecordArray[$i]['regNo']= $span1.$studentRecordArray[$i]['regNo'].$span2 ;
   	   $srNo = $span1.($records+$i+1).$span2 ;
	}

        $valueArray = array_merge(array('act' =>  $showlink, 'srNo' => $srNo ),$studentRecordArray[$i]);
