...
UtilityManager::headerNoCache();
   ...
   if (!isset($REQUEST_DATA['subjectTypeCode']) || trim($REQUEST_DATA['subjectTypeCode']) == '') {
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = SubjectTypeManager::getInstance()->getSubjectType(' WHERE UCASE(subjectTypeName)="'.add_slashes(strtoupper($REQUEST_DATA['subjectTypeName'])).'" AND universityId = "'.$REQUEST_DATA['universityId'].'" AND subjectTypeId!='.$REQUEST_DATA['subjectTypeId']);
        if(trim($foundArray[0]['subjectTypeCode'])=='') {  //DUPLICATE CHECK
            $foundArray = SubjectTypeManager::getInstance()->getSubjectType(' WHERE UCASE(subjectTypeCode)="'.add_slashes(strtoupper($REQUEST_DATA['subjectTypeCode'])).'" AND universityId = "'.$REQUEST_DATA['universityId'].'" AND subjectTypeId!='.$REQUEST_DATA['subjectTypeId']);
        if(trim($foundArray[0]['subjectTypeCode'])=='') {  //DUPLICATE CHECK        
            $returnStatus = SubjectTypeManager::getInstance()->editSubjectType($REQUEST_DATA['subjectTypeId']);            
