...
//--------------------------------------------------------
    ...
    $totalArray      = $bookMgr->getBookDispatchList($classId);
    $studentRecordArray = $bookMgr->getBookDispatchList($classId,$filter,$limit,$orderBy);
    $cnt = count($studentRecordArray);
    
    for($i=0;$i<$cnt;$i++) {
        
       $checked='';
       if($studentRecordArray[$i]['status']==BOOK_DISPACHED){
