...
//--------------------------------------------------------
	...
	$selectedCompanyId = $sessionHandler->getSessionVariable('CompanyId');
    for($i=0;$i<$cnt;$i++) {
        $thisCompanyId = $companyRecordArray[$i]['companyId'];

		$action1 = '<a href="#" title="Select Company"><img src="'.IMG_HTTP_PATH.'/select.gif" border="0" alt="Select Company" onclick="selectCompany('.$companyRecordArray[$i]['companyId'].')"></a>';

		if (intval($thisCompanyId) == intval($selectedCompanyId)) {
			$action1 = '<img src="'.IMG_HTTP_PATH.'/selected.gif" border="0" alt="Selected Company" />';
		}

        $valueArray = array_merge(array('action1' =>  $action1, 'srNo' => ($records+$i+1) ),$companyRecordArray[$i]);
...
// $History: ajaxInitSelectList.php $
//
//*****************  Version 2  *****************
