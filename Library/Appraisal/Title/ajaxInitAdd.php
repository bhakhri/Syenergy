...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = TitleManager::getInstance()->getTitle(' WHERE ( UCASE(appraisalTitle)="'.add_slashes(strtoupper(trim($REQUEST_DATA['titleName']))).'" )');
        
        if(trim($foundArray[0]['appraisalTitle'])=='') {  //DUPLICATE CHECK
            $returnStatus = TitleManager::getInstance()->addTitle();
