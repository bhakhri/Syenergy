...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = PayrollManager::getInstance()->getFromYear(' WHERE UCASE(startYear)="'.add_slashes(trim(strtoupper($REQUEST_DATA['fromYear']))).'" AND financialYearId!='.$REQUEST_DATA['yearId']);

		if(trim($foundArray[0]['startYear'])=='') {  //DUPLICATE Bank NAME CHECK
				$returnStatus = PayrollManager::getInstance()->editYear($REQUEST_DATA['yearId']);
