...
//--------------------------------------------------------
    ...
    if(UtilityManager::notEmpty($REQUEST_DATA['searchbox'])) {
       $filter = '  WHERE financialYear LIKE "'.add_slashes($REQUEST_DATA['searchbox']).'%"';
    }
    $sortOrderBy = (UtilityManager::notEmpty($REQUEST_DATA['sortOrderBy'])) ? $REQUEST_DATA['sortOrderBy'] : 'DESC';
    $sortField = (UtilityManager::notEmpty($REQUEST_DATA['sortField'])) ? $REQUEST_DATA['sortField'] : 'financialYear';
    
     $orderBy = " $sortField $sortOrderBy";         

    
    $totalArray = $payrollManager->getTotalStructures($filter);
    $structureRecordArray = $payrollManager->getStructureList($filter,$limit,$orderBy);
    $cnt = count($structureRecordArray);
    
    for($i=0;$i<$cnt;$i++) { 

        $valueArray = array_merge(array('action' =>  $structureRecordArray[$i]['structureId'] , 'srNo' => ($records+$i+1) ),$structureRecordArray[$i]);
