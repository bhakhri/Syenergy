...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
			...
			if ($accidentDate1 == '' || $accidentDate1 == '0000-00-00') {
				echo SELECT_ACCIDENT_DATE;
				die;
			}
			if($busNo != '') {  //DUPLICATE CHECK
				if(SystemDatabaseManager::getInstance()->startTransaction()) {
				$returnStatus = $vehicleAccidentManager->editVehicleAccident($REQUEST_DATA['accidentId']);
			...
			if(trim($foundArray1[0]['busNo'])==$REQUEST_DATA['busNo']){
             echo BUS_ALREADY_INSURED;
             die;
            }
		}
	}
