...
UtilityManager::headerNoCache();
    ...
    if (trim($errorMessage) == '') {
        ...
        $foundArray = FeedBackManager::getInstance()->getFeedBackQuestions(' WHERE feedbackSurveyId='.$REQUEST_DATA['labelId'].' AND UCASE(feedbackQuestion)="'.add_slashes(strtoupper($REQUEST_DATA['questionTxt'])).'" AND feedbackQuestionId !='.$REQUEST_DATA['feedbackQuestionId']);
        if(trim($foundArray[0]['feedbackQuestion'])=='') {  //DUPLICATE CHECK
            $returnStatus = FeedBackManager::getInstance()->editFeedBackQuestions($REQUEST_DATA['feedbackQuestionId']);
