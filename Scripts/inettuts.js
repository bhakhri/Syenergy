...
var iNettuts = {
            ...
            if (thisWidgetSettings.removable) {
                $('<a href="#" class="remove">CLOSE</a>').mousedown(function (e) {
                    ...
                    //if(confirm('This widget will be removed, are you sure?')) {
                        try{
                          //document.getElementById(thisId+'_1').disabled=false;
                          //document.getElementById(thisId+'_2').disabled=true;
                          document.getElementById(thisId+'_1').checked=false
                        }
                        catch(e){
                        }
    ...
    savePreferences : function () {
        ...
        /*alert(cookieString);*/
        getData(themeFilePathURL+'/ChangePreference/ajaxStoreDashboard.php', 'dashboardLayout=' + escape(cookieString)+'!~!'+escape(cookieString2));
    },
    ...
    sortWidgets : function () {
        ...
        //value fetched from database instead of cookie
        var cookie=inettus_data;
        if(cookie==-1) {
        ...
        $(settings.columns).each(function(i){
            ...
            $(widgetData).each(function(){
                ...
                //$(clonedWidget).find('h3:eq(0)').html(thisWidgetData[2].replace(/\[-PIPE-\]/g,'|').replace(/\[-COMMA-\]/g,','));
                    ...
                    //alert(thisWidgetData[3]+'---'+clonedWidget);
    ...
    addWidget : function (where, opt) {
      ...
      showWaitDialog(true);
      document.getElementById(where).innerHTML +=iNettuts.initWidget(opt);
      //$(where).append(iNettuts.initWidget(opt));
      hideWaitDialog(true); 
    ...
    initWidget : function (opt) {
      $ = $j;
      var divData=noDataFoundVar;
      $.ajax({
                url: themeFilePathURL+"/Teacher/getWidgetsData.php?id="+opt.id,
                async :false,
                success: function(data) {
                  divData=trim(data);
                }
      }); 
      return '<li id="'+opt.id+'" class="new widget '+opt.color+'"><div class="widget-head"><h3 class="contenttab_internal_rows1"><font color="#FFFFFF">'+opt.title+'</font></h3></div><div class="widget-content">'+divData+'</div></li>';
    }
