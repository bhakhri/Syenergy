...
//--------------------------------------------------------
	...
	//branch Code
	$hostelCode = $REQUEST_DATA['branchCode'];

	//selected Year
	$selectedYear = $REQUEST_DATA['selectedYear'];

	$conditions = "";
	if($hostelCode=='hostel')
		$conditions .= " AND stu.hostelId IS NOT NULL";

	if($hostelCode=='day scholar')
		$conditions .= " AND stu.hostelId IS NULL";

	/* END: search filter */
	$conditions .= "  AND YEAR(dateOfAdmission)= $selectedYear";
    $recordArray = $studentAdmissionManager->getHostelList($conditions,'',' cls.className DESC');
	...
	$reportManager->setReportHeading('Student List for '.$hostelCode.' for '.$selectedYear.' Year');
...
// $History: studentHostelYearReportPrint.php $
//
//*****************  Version 1  *****************
