...
//--------------------------------------------------------
	...
	//branch Code
	$degreeCode = $REQUEST_DATA['branchCode'];

	//selected Year
	$selectedYear = $REQUEST_DATA['selectedYear'];
	
	/* END: search filter */
	$conditions = "  AND deg.degreeCode ='$degreeCode' AND YEAR(dateOfAdmission)= $selectedYear";
    $recordArray = $studentAdmissionManager->getDegreeList($conditions,'',' cls.className DESC');
	...
	$reportManager->setReportHeading('Student List for '.$degreeCode.' Degree for '.$selectedYear.' Year');
...
// $History: studentDegreeYearReportPrint.php $
//
//*****************  Version 1  *****************
