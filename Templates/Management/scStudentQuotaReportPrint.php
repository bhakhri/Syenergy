<?php 
//This file is used as printing version for quota wise student report for management role.
//
...
//--------------------------------------------------------
	...
	/* END: search filter */
	$conditions = "  AND sc.quotaId =$quotaId";
    $recordArray = $dashboardManager->getStudentReportList($conditions,$orderBy);
    ...
    for($i=0;$i<$cnt;$i++) {
		...
		$quotaName = $recordArray[$i]['quotaName'];
	...
	$reportManager->setReportHeading('Student List for '.$quotaName.' Category');
...
// $History: scStudentQuotaReportPrint.php $
//
//*****************  Version 1  *****************
