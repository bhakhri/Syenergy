...
//--------------------------------------------------------
	...
	//branch Code
	$quotaCode = $REQUEST_DATA['branchCode'];

	//selected Year
	$selectedYear = $REQUEST_DATA['selectedYear'];
	
	/* END: search filter */
	$conditions = "  AND qta.quotaAbbr ='$quotaCode' AND YEAR(dateOfAdmission)= $selectedYear";
    $recordArray = $studentAdmissionManager->getQuotaList($conditions,'',' cls.className DESC');
	...
	$reportManager->setReportHeading('Student List for '.$quotaCode.' Category for '.$selectedYear.' Year');
...
// $History: studentCategoryYearReportPrint.php $
//
//*****************  Version 1  *****************
