...
function parseName($value){
  ...
  if($type==1){  
    ...
    for($j=0;$j<count($studentRecordArray);$j++) {
       $userId = $studentRecordArray[$j]['userId']; 
       $find=1;
   ...
   // $studentRecordArray=$fbManager->getStudentsFeedbackStatus($conditions,$labelId,$orderBy,'',$timeTableLabelId);
       $actionString='';
       
       if($type==1){ //for students
          if($studentRecordArray[$j]['dateOverFlag']==1){
             if($studentRecordArray[$j]['isCompletedFlag']!=1){
                if($studentRecordArray[$j]['currentStatus']==FEEDBACK_STUDENT_BLOCKED){
                   $studentRecordArray[$j]['currentStatus']='Blocked';
                   $actionString='<a href="javascript:void(0);" title="Unblock this student"><img style="height:20px;" src="'.IMG_HTTP_PATH.'/selected.gif" onclick="changeStatus('.$studentRecordArray[$j]['studentId'].','.FEEDBACK_STUDENT_FORCED_UNBLOCKED.')" ></a>';
                }
                else if($studentRecordArray[$j]['currentStatus']==FEEDBACK_STUDENT_UNBLOCKED){
                   $studentRecordArray[$j]['currentStatus']='Not Blocked';
                   $actionString='<a href="javascript:void(0);" title="Block this student"><img style="height:20px;" src="'.IMG_HTTP_PATH.'/block_user.png" onclick="changeStatus('.$studentRecordArray[$j]['studentId'].','.FEEDBACK_STUDENT_BLOCKED.');" ></a>';
                }
                else{
           ...
           $csvData .= ($j+1).",".parseCSVComments($studentRecordArray[$j]['studentName']);
           $csvData .= ",".parseCSVComments($studentRecordArray[$j]['rollNo']).",".parseCSVComments($studentRecordArray[$j]['universityRollNo']);
           $csvData .= ",".parseCSVComments($studentRecordArray[$j]['className']).",".parseCSVComments($studentRecordArray[$j]['isCompleted']);
           $csvData .= ",".parseCSVComments($studentRecordArray[$j]['dateOver']).",".parseCSVComments($studentRecordArray[$j]['currentStatus']);
           $csvData .= "\n";
        }
    }
