...
function parseCSVComments($comments) {
  ...
  if(eregi(",", $comments) or eregi("\n", $comments)) {
   ...
   $fbMgr = FeedBackCategoryAdvancedManager::getInstance();
	...
	$csvData = '';
    $csvData .= "#,Category,Parent Category,Relationship,Subject Type,Print Order\n";
	foreach($valueArray as $record) {
        $csvData .= $record['srNo'].','.parseCSVComments($record['feedbackCategoryName']).','.parseCSVComments($record['parentCategoryName']).','.parseCSVComments($record['feedbackType']).','.parseCSVComments($record['subjectTypeName']).','.parseCSVComments($record['printOrder']);
		$csvData .= "\n";
	}
	if($cnt==0){
	...
	header('Content-Disposition: attachment;  filename="feedbackCategoryReport.csv"');
	header("Content-Transfer-Encoding: binary\n");
	echo $csvData;
	die;    
// $History: feedBackCategoryCSV.php $
//
//*****************  Version 1  *****************
