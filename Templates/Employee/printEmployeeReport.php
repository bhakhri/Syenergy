   ...
   if(UtilityManager::notEmpty($REQUEST_DATA['searchbox'])) {
       ...
       elseif(strtolower(trim($REQUEST_DATA['searchbox']))=='yes' || strtolower(trim($REQUEST_DATA['searchbox']))=='ye') {
        ...
        $filter = ' AND (emp.employeeName LIKE "%'.add_slashes(trim($REQUEST_DATA['searchbox'])).'%" OR emp.employeeCode LIKE "%'.add_slashes(trim($REQUEST_DATA['searchbox'])).'%" OR 
                         ...
                         emp.guestFaculty LIKE "%'.$type.'%" OR
                         emp.isTeaching LIKE "%'.$type.'%" )' ; 
    }
	...
	$employeeArray = $employeeManager->getEmployeeList($filter,'',$orderBy); 
                            ...
                            if($recordCount >0 && is_array($employeeArray) ) { 
                                ...
                                for($i=0; $i<$recordCount; $i++ ) {
                                   ...
                                   /* $marksPrintArray[$i]['srNo']   =$j;
                                    ...
                                    if(trim($employeeArray[$i]['employeeCode'])==''){
                                        $employeeArray[$i]['employeeCode']=NOT_APPLICABLE_STRING;
                                    }
    ...
    $reportManager->setReportHeading('Employee Report');
	if ($search != '') {
    ...
    $reportTableHead['employeeName']		           =    array('Name ',         ' width=18% align="left" ','align="left" ');
	...
	$reportTableHead['emailAddress']		           =    array('Email',        'width="12%" align="left"','align="left"');
    $reportTableHead['guestFacultyDisplay']            =    array('Guest Faculty',        'width="10%" align="left"','align="left"');  
