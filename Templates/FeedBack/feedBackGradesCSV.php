...
function parseCSVComments($comments) {
 ...
 return $comments; 
	...
	$csvData = '';
    $csvData .= "Sr, Grade Label, Grade Value, Feedback Label \n";
	foreach($valueArray as $record) {
        $csvData .= $record['srNo'].', '.parseCSVComments($record['feedbackGradeLabel']).', '.parseCSVComments($record['feedbackGradeValue']).', '.parseCSVComments($record['feedbackSurveyLabel']);
		$csvData .= "\n";
	}
	ob_end_clean();
	...
	header('Content-Disposition: attachment;  filename="feedBackGradesReport.csv"');
	header("Content-Transfer-Encoding: binary\n");
	echo $csvData;
	die;    
 

// $History: feedBackGradesCSV.php $
//
//*****************  Version 1  *****************
