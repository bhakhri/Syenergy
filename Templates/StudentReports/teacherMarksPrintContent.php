...
//--------------------------------------------------------
	...
	if (!empty($subjectCode)) {
		$conditions .= " AND s.subjectId = $subjectCode";
	}
	if (!empty($subjectType)) {
		$conditions .= " AND s.subjectTypeId = $subjectType";
	}
	if (!empty($timeTableLabelId)) {
		$conditions .= " AND ttl.timeTableLabelId = $timeTableLabelId";
	}
	$intervalArr = explode("-", $interval);
 

// AND ttm.classId =10 AND s.subjectTypeId =2 AND ttl.timeTableLabelId =1 AND s.subjectId =13

	$studentRecordArray = $studentReportsManager->getAllTeacherWiseDetails($conditions,$intervalArr[0],$intervalArr[1],"stu.firstName");
	...
	$reportTableHead['maxMarks1']	=	array('Marks Scored','width="10%" align="right"', 'align="right"');
