...
$studentPendingDuesManager = StudentPendingDuesManager::getInstance();
    ...
    $cnt=count($finalPindingArray);
    $csvData = "#, Student Name, COLUMN_ROLL_NO, COLUMN_UNIV_ROLL_NO,Class,Mobile,Academic,Transport,Hostel,Prev. Dues,Total \n"; 
for($i=0;$i<$cnt;$i++) {
   $csvData .= ($i+1).",".parseCSVComments($finalPindingArray[$i]['studentName']).",".parseCSVComments($finalPindingArray[$i]['rollNo']);
   $csvData .= ",".parseCSVComments($finalPindingArray[$i]['universityRollNo']).",".parseCSVComments($finalPindingArray[$i]['feeClassName']);
   $csvData .= ",".parseCSVComments($finalPindingArray[$i]['studentMobileNo']).",".parseCSVComments($finalPindingArray[$i]['academicDues']);
   $csvData .= ",".parseCSVComments($finalPindingArray[$i]['transportDues']).",".parseCSVComments($finalPindingArray[$i]['hostelDues']);
   $csvData .= ",".parseCSVComments($finalPindingArray[$i]['prevDues']).",".parseCSVComments($finalPindingArray[$i]['total'])." \n";  
}
...
UtilityManager::makeCSV($csvData,'StudentPendingDues.csv');
