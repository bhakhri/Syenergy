...
//--------------------------------------------------------
    ...
    $reportHead .= "Date,".UtilityManager::formatDate($REQUEST_DATA['fromDate']).",to,".UtilityManager::formatDate($REQUEST_DATA['toDate'])."\n";

    $csvData .= $reportHead;
    $csvData .= "#, Date, Sender, Subject, Type, Receiver, Brief Description \n";
	$count = COUNT($valueArray);
	if($count == 0){
		$csvData .= "NO Data Found";
	}
    foreach($valueArray as $record) {
        $csvData .= $record['srNo'].','.$record['dated'].','.$record['userName'].','.$record['subject'].','
