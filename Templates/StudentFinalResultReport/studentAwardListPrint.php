<?php 
   ...
   $studentCGPARecordArray = $studentReportsManager->getStudentClasswiseCGPA($condition);
    ...
    if ($gradeList == 'on') {
        ...
        $tableData ="<table border='1' cellpadding='0px' cellspacing='2px' width='100%' class='reportTableBorder'  align='center'> 
                        <tr>
                            <td ".$reportManager->getReportDataStyle()." rowspan='2' align='left'><b>#</b></td>
                            <td ".$reportManager->getReportDataStyle()." rowspan='2' align='left'><b>Roll No.</b></td>
                            <td ".$reportManager->getReportDataStyle()." rowspan='2' align='left'><b>Name</b></td>";
        for($i=0;$i < count($subjectArray); $i++) {
           $subjectId = $subjectArray[$i]['subjectId'];
           $subjectCode = $subjectArray[$i]['subjectCode'];
           $tableData .="<td ".$reportManager->getReportDataStyle()." align='center' colspan='$colCounter'><b>$subjectCode</b></td>";          
        } 
        $tableData .="</tr> <tr>";  
        ...
        for($i=0;$i < count($subjectArray); $i++) {
            ...
            $tableData .="<td ".$reportManager->getReportDataStyle()." align='center'><b>Internal Marks<br>($ttMax2)</b></td>"; 
            $tableData .="<td ".$reportManager->getReportDataStyle()." align='center'><b>External Marks<br>($ttMax1)</b></td>"; 
            $tableData .="<td ".$reportManager->getReportDataStyle()." align='center'><b>Total Marks<br>($ttSum)</b></td>"; 
            $tableData .="<td ".$reportManager->getReportDataStyle()." align='center'><b>Grade</b></td>"; 
        }
        $tableData .="</tr>";
        ...
        for($i=0;$i < count($studentArray); $i++) {  
            ...
            $srNo = ($records+$i+1);
            $tableData ="<tr>
                            <td ".$reportManager->getReportDataStyle()." > $srNo </td>
                            <td ".$reportManager->getReportDataStyle()." > $rollNo</td>
                            <td ".$reportManager->getReportDataStyle()." > $studentName</td>";   
            ...
            for($j=0;$j < count($subjectArray); $j++) {
                ...
                if($mm=='') {
                  $tableData .="<td ".$reportManager->getReportDataStyle()." align='center'>".NOT_APPLICABLE_STRING."</td>";   
                }
                else {
                  $tableData .="<td ".$reportManager->getReportDataStyle()." align='center'>$mm</td>";   
                ...
                if($totalMks=='') {
                  ...
                  $tableData .="<td ".$reportManager->getReportDataStyle()." align='center'>$totalMks</td>";   
                  // Grading                 
        ...
        $cntPage = count($finalArray)/$pageRecordLimit;
        ...
        for($i=0;$i<count($finalArray);$i++) {
           ...
           $find++;    
           $tableData .= $finalArray[$i]; 
           if($find==$pageRecordLimit) {
              $tableData .= "</table>";  
              reportGenerate($tableData,$heading,$univName); 
              $find='0';
              $tableData = '';
           }
        }
...
die;
 ...
 // Report generate
    function reportGenerate($value,$heading,$univName,$isFooter='0') {
        ...
        $reportManager->setReportHeading('Student Award Report');
        $reportManager->setReportInformation("$heading");     
        
        
        $ttFooter = "<table border='0' cellspacing='2px' cellpadding='2px' width='20%' align='left'>
                     <tr>
       ...
       ?>     
        ...
        <table border="0" cellspacing="0" cellpadding="0" width="800px" align="center">
          <tr>
            <td align="left" colspan="1" width="100%" class="">
