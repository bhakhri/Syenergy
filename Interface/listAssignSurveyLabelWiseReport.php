...
function getStudentList(queryString,mySortBy,mySortOrder,myPage){
  ...
  var studentListUrl = '<?php echo HTTP_LIB_PATH;?>/FeedbackAdvanced/ajaxStudentFeedbackLabelWiseReport.php'; 
  var tableColumns = new Array(
                         ...
                         new Array('dateOver','Date Over','width="6%"',false),
                         new Array('currentStatus','Status','width="8%"',false),
 ...
 listObj = new initPage(studentListUrl,recordsPerPage,linksPerPage,myPage,frmName,mySortBy,mySortOrder,'studentResultsDiv','','',true,'listObj',tableColumns,'','','&timeTableLabelId='+document.getElementById('timeTableLabelId').value+'&labelId='+document.getElementById('labelId').value+'&type=1');
...
function showEmployeeList(queryString){
  ...
  var empListUrl = '<?php echo HTTP_LIB_PATH;?>/FeedbackAdvanced/ajaxStudentFeedbackLabelWiseReport.php'; 
...
function doChangeStatus(userId,mode,reason){
    ...
    var url = '<?php echo HTTP_LIB_PATH;?>/FeedbackAdvanced/changeStudentStatus.php';
    new Ajax.Request(url,
           {
             method:'post',
             parameters: {
...
function blockAllStudents(){
    var totalStudentCountForBlocking=listObj.totalRecords;
    if(totalStudentCountForBlocking>0){
        if(!confirm(totalStudentCountForBlocking+"(s) students will be blocked.\nAre you sure?")){
            return false;
        }
    }
    else{
        messageBox("No students found for blocking");
        return false;
    }
    
    var url = '<?php echo HTTP_LIB_PATH;?>/FeedbackAdvanced/blockAllStudents.php';
    new Ajax.Request(url,
           {
             method:'post',
             ...
             onSuccess: function(transport){
                     hideWaitDialog(true);
                     if("<?php echo SUCCESS;?>" == trim(transport.responseText)) {
                        messageBox(totalStudentCountForBlocking+"(s) students are blocked now");   
                        getStudentList('',listObj.sortField,listObj.sortOrderBy,listObj.page);
                     }
                     else {
                        messageBox(trim(transport.responseText));
...
function unBlockAllStudents(){
    var totalStudentCountForUnBlocking=listObj.totalRecords;
    if(totalStudentCountForUnBlocking>0){
        if(!confirm(totalStudentCountForUnBlocking+"(s) students will be unblocked.\nAre you sure?")){
            return false;
        }
    }
    else{
        messageBox("No students found for unblocking");
        return false;
    }
    ...
    var url = '<?php echo HTTP_LIB_PATH;?>/FeedbackAdvanced/unBlockAllStudents.php';
    new Ajax.Request(url,
           {
             method:'post',
             ...
             onSuccess: function(transport){
                     hideWaitDialog(true);
                     if("<?php echo SUCCESS;?>" == trim(transport.responseText)) {
                        messageBox(totalStudentCountForUnBlocking+"(s) students are unblocked now");
...
function printReport() {
   ...
   path='<?php echo UI_HTTP_PATH;?>/assignSurveyLabelWiseReportPrint.php?'+qstr+'&timeTableLabelId='+document.getElementById('timeTableLabelId').value+'&labelId='+document.getElementById('labelId').value+'&type='+type;
...
function printCSV() {
   ...
   window.location='assignSurveyLabelWiseReportCSV.php?'+qstr+'&timeTableLabelId='+document.getElementById('timeTableLabelId').value+'&labelId='+document.getElementById('labelId').value+'&type='+type;;
}
...
<body>
	...
	require_once(TEMPLATES_PATH . "/FeedbackAdvanced/listAssignSurveyLabelWiseReportsContents.php");
