...
<title><?php echo SITE_NAME;?>:  Define Concession Value</title>
<?php 
...
function addOneRow(cnt)
{
     ...
     createRows(resourceAddCnt,cnt,eval("document.getElementById('feeHead').innerHTML"),eval("document.getElementById('category').innerHTML"));
}
...
//create dynamic rows 
function createRows(start,rowCnt,feeHeadData){
     ...
     for(var i=0;i<rowCnt;i++) {
          ...
          var txt4a=document.createElement('select');
       ...
       */   
          ...
          txt1.setAttribute('id','categoryId'+parseInt(start+i,10));
          txt1.setAttribute('name','categoryId[]'); 
          txt1.setAttribute('style','width:270px');    
          txt1.className='htmlElement';
          
          txt2.setAttribute('id','feeHeadId'+parseInt(start+i,10));
          txt2.setAttribute('name','feeHeadId[]'); 
          ...
          txt4a.setAttribute('id','isConcessionType'+parseInt(start+i,10));
          txt4a.setAttribute('name','isConcessionType[]'); 
          txt4a.setAttribute('style','width:60px');    
          txt4a.className='htmlElement';
       ...
       */   
          ...
          var categoryLen= document.getElementById('category').options.length;
          var t=document.getElementById('category');
          // add option Select initially
          if(categoryLen>0) {
             var tt='categoryId'+parseInt(start+i,10); 
             //alert(eval("document.getElementById(tt).length"));
          ...
          var tt='isConcessionType'+parseInt(start+i,10); 
          addOption(document.getElementById(tt), "1", "%age");
...
function validateAddForm(frm) {
    ...
    if(resourceAddCnt==0) {
        ...
        msg = confirm('All Define Concession Value will be deleted. Are you sure?')
        if(msg == false) {
            return false;
        }
    }
            ...
            for(var i=0;i<total;i++) {
                if(obj[i].type.toUpperCase()=='HIDDEN' && obj[i].name.indexOf('idNos[]')>-1) {
                   // blank value check 
                   id =obj[i].value;
                   if(eval("document.getElementById('categoryId"+id+"').value")=='') {
                     messageBox ("Select category");  
                     eval("document.getElementById('categoryId"+id+"').focus()");
                     return false;             
                   }
                   ...
                   if(eval("document.getElementById('isLeet"+id+"').value")=='') {
                     messageBox ("Select IsLeet ");  
                     eval("document.getElementById('isLeet"+id+"').focus()");
                     return false;             
                   }
                                      
              
                   chkDuplicateRecord = eval("document.getElementById('categoryId"+id+"').value");
                   chkDuplicateRecord = chkDuplicateRecord + "~"+eval("document.getElementById('feeHeadId"+id+"').value");
                   ...
                   if(trim(eval("document.getElementById('totalAmount"+id+"').value"))=='') {                          
                     messageBox ("Enter numeric value for concession");
                   ...
                   if(eval("document.getElementById('isConcessionType"+id+"').value")=='1') {
                        if(eval("document.getElementById('totalAmount"+id+"').value")<0 || eval("document.getElementById('totalAmount"+id+"').value")>100)   {           messageBox ("Value should be between 0 to 100");
                          eval("document.getElementById('totalAmount"+id+"').focus()");  
                          return false;
                       }
                    }
...
function showFeeHeadValue() {
   ...
   var  url = '<?php echo HTTP_LIB_PATH;?>/ClassWiseHeadMapping/ajaxGetClassWiseValues.php';
   document.getElementById("totAmount").innerHTML = "<?php echo NOT_APPLICABLE_STRING; ?>"; 
   var showDelete = '';  
   document.getElementById('trAttendance').style.display='none';    
   cleanUpTable();   
               ...
               hideWaitDialog(true);
                ...
                if(len>0) {
                  ...
                  for(var i=0;i<len;i++) {
                    ...
                    var id = "categoryId"+varFirst;
                    eval("document.getElementById(id).value = j[i]['categoryId']");   
                    
                    id = "feeHeadId"+varFirst;
                    if(j[i]['feeHeadId']=='') {
                    ...
                    id = "isConcessionType"+varFirst;
                    eval("document.getElementById(id).value = j[i]['concessionType']");

                    id = "totalAmount"+varFirst;
                    eval("document.getElementById(id).value = j[i]['concessionAmount']");
                  }
               } 
         },
...
function addFeeHeadValue() {
   ...
   var url = '<?php echo HTTP_LIB_PATH;?>/ClassWiseHeadMapping/ajaxClassWiseMappingAdd.php';
...
*/
   ...
   new Ajax.Request(url,
   {
     ...
     onSuccess: function(transport){
        hideWaitDialog(true);    
        if(trim("<?php echo DEFINE_CONCESSION_VALUE_ADDED;?>") == trim(transport.responseText) || trim("<?php echo DEFINE_CONCESSION_VALUE_DELETE;?>") == trim(transport.responseText) || trim("<?php echo DEFINE_CONCESSION_VALUE_UPDATED;?>") == trim(transport.responseText)) {
...
function addFeeHeadValuesCopy() {
  ...
  var url = '<?php echo HTTP_LIB_PATH;?>/ClassWiseHeadMapping/ajaxClassWiseMappingCopy.php';
  ...
  if(trim(document.getElementById('souClassId').value)==""){
   ...
   if(false===confirm("Are you sure to Copy Define Concession Value?")) {
     return false;
   }
   else { 
       params = generateQueryString('allDetailsForm');  
...
function printReport() {
    path='<?php echo UI_HTTP_PATH;?>/listClassWiseMappingValuesPrint.php?classId='+document.getElementById('classId').value;
...
function printReportCSV() {
    path='<?php echo UI_HTTP_PATH;?>/listClassWiseMappingValuesCSV.php?classId='+document.getElementById('classId').value;     
...
<body>
    ...
    require_once(TEMPLATES_PATH . "/ClassWiseHeadMapping/classWiseMappingContents.php");
