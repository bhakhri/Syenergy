...
searchFormName = 'dayBookForm'; // name of the form which will be used for search
...
function printReport() {
	form = document.dayBookForm;
	path='<?php echo UI_HTTP_ACCOUNTS_PATH;?>/dayBookPrint.php?tillDate='+form.tillDate.value;
	window.open(path,"DayBookPrint","status=1,menubar=1,scrollbars=1, width=900");
}


function showDayBook(tillDate) {
	var url = '<?php echo HTTP_LIB_PATH;?>/Accounts/DayBook/initDayBook.php';
	frm = document.dayBookForm;
	var pars = generateQueryString('dayBookForm');
...
window.onload = function() {
	...
	if (typeof tillDate != 'undefined') {
		validateAddForm(document.dayBookForm, tillDate)
	}
}
...
<body>
   ...
   echo '<script type="text/javascript">currentThemeId = "'.$sessionHandler->getSessionVariable('UserThemeId').'";</script>'; 
    require_once(TEMPLATES_PATH . "/Accounts/DayBook/listDayBookReport.php");
	echo '<script language="javascript">changeColor(currentThemeId);</script>';
    ?>
</body>
</html>
<?php 
// $History: listDayBook.php $
//
//*****************  Version 1  *****************
