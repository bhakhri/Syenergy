<?php
//-------------------------------------------------------
//  This File contains intial code for balance sheet.
//
//
...
searchFormName = 'balanceSheetForm'; // name of the form which will be used for search
...
function printReport() {
	form = document.balanceSheetForm;
	path='<?php echo UI_HTTP_ACCOUNTS_PATH;?>/balanceSheetReportPrint.php?tillDate='+form.tillDate.value;
	window.open(path,"BalanceSheetReport","status=1,menubar=1,scrollbars=1, width=900");
}


function showBalanceSheet(tillDate) {
	var url = '<?php echo HTTP_LIB_PATH;?>/Accounts/BalanceSheet/initBalanceSheet.php';
	frm = document.balanceSheetForm;
	var pars = generateQueryString('balanceSheetForm');
...
window.onload = function() {
	...
	if (typeof tillDate != 'undefined') {
		validateAddForm(document.balanceSheetForm, tillDate)
	}
}
...
<body>
   ...
   echo '<script type="text/javascript">currentThemeId = "'.$sessionHandler->getSessionVariable('UserThemeId').'";</script>'; 
    require_once(TEMPLATES_PATH . "/Accounts/BalanceSheet/listBalanceSheetReport.php");
	echo '<script language="javascript">changeColor(currentThemeId);</script>';
    ?>
</body>
</html>
<?php 
// $History: listBalanceSheet.php $
//
//*****************  Version 1  *****************
