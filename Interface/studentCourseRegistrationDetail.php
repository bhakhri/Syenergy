...
require_once(BL_PATH ."/Student/initStudentInformation.php");
?>
        ...
        function createElectiveRows(start,rowCnt,mode,subjectData,courseType){
              ...
              for(var i=0;i<rowCnt;i++) {
                 ...
                 // var txt0=document.createTextNode(i+1);
                  ...
                  if(courseType=='E') { 
                    //txt1.setAttribute('onchange',"getCredits(this,'E')");  
                    id1 = "electiveCreditId"+mode;
                    id2 = "electiveCreditId"+mode+'[]';
                  }
                  else if(courseType=='C') { 
                    //txt1.setAttribute('onchange',"getCredits(this,'C')");  
...
function getCredits(credit,mode) {
   var id = credit.id;
   var c = credit.value;
   var rval=c.split('~'); 
       ...
       for(i=0;i<tot;i++) {   
          ...
          for(var j=0;j<total;j++) {
              ...
              if(obj[j].type.toUpperCase()=='TEXT' && eval("obj[j].name.indexOf('"+id+"')") >-1) {
                   ...
                   // Integer Value Checks updated
                  // if(!isInteger(trim(obj[j].value))) {
                  //   messageBox ("Enter the integer value for "+msg.toLowerCase()+" course credits from (0 to 10)");
                  //   obj[j].focus();
                  //   return false;
                  // }
                   
                   // Ranges Checks 
                   if(parseInt(obj[j].value,10) < 0 || parseInt(obj[j].value,10) >10 ) {
                     messageBox ("Enter the integer value for "+msg.toLowerCase()+" course credits from (0 to 10)");
                     obj[j].focus();
                     return false;
                   }
...
function populateStudentRegistrationValue() {
    ...
    url = '<?php echo HTTP_LIB_PATH;?>/Student/ajaxGetRegistrationValue.php';   
...
function printReport() {
    ...
    cId = document.getElementById('currentClassId').value;
...
<body>
    ...
    require_once(TEMPLATES_PATH . "/StudentReports/listCourseRegistrationDetailContents.php");
