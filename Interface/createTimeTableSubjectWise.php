...
define('MODULE','CreateTimeTableSubjectWiseAdvanced');
define('ACCESS','edit');
...
<title><?php echo SITE_NAME;?>: Subject Wise (Weekly) Time Table</title>
<style>
.htmlElementGroup
{
...
function validatetTimetableForm(str) {
	...
	var fieldsArray = new Array(new Array("periodSlotId","Please select period slot"),
                                ...
                                new Array("subjectHidden","Please select subject")
                                );
...
function addTimeTable(str) {
   if (str == 'conflicts') {
	   url = '<?php echo HTTP_LIB_PATH;?>/TimeTable/ajaxInitSubjectWiseAddAdvanced.php';
   }
   else {
	   url = '<?php echo HTTP_LIB_PATH;?>/TimeTable/ajaxInitSubjectWiseSaveAdvanced.php';
   }
...
function getGroups(ctr) {
	...
	selectedSubject = document.getElementById('subjectHidden').selectedIndex;
	if (selectedSubject == 0) {
		return false;
	}
	selectedSubjectId = document.getElementById('subjectHidden').value;
...
function getCurrentTimeTable() {
    ...
    url = '<?php echo HTTP_LIB_PATH;?>/TimeTable/getClassSubjectTimeTableAdvanced.php';
    if(form.studentClass.value=="") {
       return false;
    }
	pars = 'classId='+form.studentClass.value+'&labelId='+form.timeTableLabelId.value+'&periodSlotId='+form.periodSlotId.value;
    pars = pars+'&subjectId='+form.subjectHidden.value;
    new Ajax.Request(url,
    {
         method:'post',
         parameters: pars,
...
<body>
    ...
    require_once(TEMPLATES_PATH . "/TimeTable/timetableSubjectWiseContents.php");
