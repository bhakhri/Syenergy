...
define('MODULE','CreateTimeTableAdvancedDateWise');
define('ACCESS','edit');
...
<title><?php echo SITE_NAME;?>: Manage Class Wise Date Wise (Daily) Time Table</title>
<style>
.htmlElementGroup
{
...
function addTimeTable(str) {
   if (str == 'conflicts') {
	   url = '<?php echo HTTP_LIB_PATH;?>/TimeTable/ajaxInitAddAdvancedDayWise.php';
   }
   else {
	   url = '<?php echo HTTP_LIB_PATH;?>/TimeTable/ajaxInitSaveAdvancedDayWise.php';
   }
			...
			hideWaitDialog(true);
			 if("<?php echo SUCCESS;?>" == trim(transport.responseText)) {
				 ...
				 if(confirm("<?php echo SUCCESS;?> \n\n <?php echo ADD_MORE; ?>")) {
					 ...
					 periodSlotIdVal = form.periodSlotId.value;
					 timeTableLabelIdVal = form.timeTableLabelId.value;
					  document.getElementById('timeTableForm').reset();
					  form.periodSlotId.value = periodSlotIdVal;
					  form.timeTableLabelId.value = timeTableLabelIdVal;
...
function createRows(start,rowCnt){
	...
	/*
	  ...
	  txt5.setAttribute('id','periods'+parseInt(start+i,10));
	  txt5.setAttribute('name','periods[]');
	  ...
	  txt12.setAttribute('title','Remove');
...
function getCurrentTimeTable() {
    ...
    url = '<?php echo HTTP_LIB_PATH;?>/TimeTable/getClassTimeTableAdvancedDayWise.php';
    if(form.studentClass.value=="") {
       return false;
    }
	pars = 'classId='+form.studentClass.value+'&labelId='+form.timeTableLabelId.value+'&periodSlotId='+form.periodSlotId.value+'&fromDate='+form.fromDate.value;
    new Ajax.Request(url,
    {
         method:'post',
         parameters: pars,
         ...
         onSuccess: function(transport){
				...
				while (currentRow < totalRows) {
					...
					monIdName = 'periods'+parseInt(currentRow,10);
...
<body>
    ...
    require_once(TEMPLATES_PATH . "/TimeTable/timetableAdvancedDayWiseContents.php");
    require_once(TEMPLATES_PATH . "/footer.php");
    ?>
</body>
</html>
<?php
// $History: createTimeTableAdvancedDayWise.php $
//
//*****************  Version 2  *****************
//User: Ajinder      Date: 4/22/10    Time: 2:39p
//Updated in $/LeapCC/Interface
//fixing time table bugs
//
//*****************  Version 1  *****************
