...
listURL = '<?php echo HTTP_LIB_PATH;?>/MedicalLeave/ajaxMedicalLeaveConflictReport.php';
...
function doMedicalLeave(){
    ...
    if(inputString==''){
     ...
     var url = '<?php echo HTTP_LIB_PATH;?>/MedicalLeave/ajaxDoMedicalLeave.php';
     new Ajax.Request(url,
       {
         ...
         onSuccess: function(transport){
                ...
                if(ret=="<?php echo SUCCESS;?>"){
                    ...
                    if(retArray[0]=='Medical Leave exceeding Maximum Limit'){
                         ...
                         if(document.getElementById('isApprove').checked==true) {
                             ...
                             listMedical = eval('('+retArray[1]+')');
                             var c1 = document.getElementById(divResultName).getElementsByTagName('SELECT');
                             var len=c1.length; 
                             for(var k=0;k<len;k++){
                                if(c1[k].name=='medicalLeaveStatus'){
                                    ...
                                    if(isStatus == "<?php echo MEDICAL_LEAVE_APPROVE; ?>") {
                                      for(var i=0; i<listMedical.length; i++){ 
		                                if(listMedical[i]['studentId'] == studentId && listMedical[i]['classId'] == classId && listMedical[i]['subjectId'] == subjectId) {
...
<?php 
	...
	require_once(TEMPLATES_PATH . "/MedicalLeave/medicalLeaveConflictReportContents.php");
