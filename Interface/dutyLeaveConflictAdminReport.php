...
listURL = '<?php echo HTTP_LIB_PATH;?>/StudentDutyMedicalLeaves/ajaxDutyLeaveConflictReport.php';
...
function doDutyLeave(){
  ...
  var elements=document.getElementById('results').getElementsByTagName('SELECT');
    ...
    for(var i=0;i<len;i++){
        if(elements[i].name=='dutyLeaveStatus'){
            if(elements[i].value=='-1'){
                messageBox("<?php echo SELECT_DUTY_LEAVE_STATUS;?>");
                elements[i].focus();
                return false;
            }
            if(inputString!=''){
    ...
    if(inputString==''){
     ...
     var url = '<?php echo HTTP_LIB_PATH;?>/StudentDutyMedicalLeaves/ajaxDoDutyLeave.php';
...
function getDutyEvent(val) {
         ...
         var url = '<?php echo HTTP_LIB_PATH;?>/StudentDutyMedicalLeaves/getDutyEvents.php';
         new Ajax.Request(url,
           {
             method:'post',
             asynchronous : false,
...
<body>
	...
	require_once(TEMPLATES_PATH . "/StudentDutyMedicalLeaves/dutyLeaveConflictReportContents.php");
