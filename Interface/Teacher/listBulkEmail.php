...
<title><?php echo SITE_NAME;?>: Send Bulk Email </title>
<?php 
...
tinyMCE.init({
       ...
       theme_advanced_buttons2 : "pastetext,pasteword,|,search,replace,|,bullist,numlist,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,anchor,cleanup,code,|,insertdate,inserttime,preview,|,forecolor,backcolor",
...
listURL = '<?php echo HTTP_LIB_PATH;?>/Teacher/TeacherActivity/ajaxBulkEmailList.php';
...
function getData(){
    if(trim(document.getElementById('studentRollNo').value)!="")
    {
       ...
       messageBox("<?php echo BULK_EMAIL_SELECT_STUDENT_LIST; ?>");
...
function validateForm() {
 ...
 if(isEmpty(trim(document.getElementById('msgSubject').value)))
 ...
 else if(isEmpty(tinyMCE.get('elm1').getContent()))
    {
     ...
     messageBox("<?php echo SELECT_STUDENT_PARENT_EMAIL; ?>");
...
//--------------------------------------------------------
function sendEmail() {
         url = '<?php echo HTTP_LIB_PATH;?>/Teacher/TeacherActivity/ajaxSendBulkEmail.php';
...
<body>
	...
	require_once(TEMPLATES_PATH . "/Teacher/TeacherActivity/listBulkEmailContents.php");
