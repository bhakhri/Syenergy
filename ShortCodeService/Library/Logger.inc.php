    ...
    public function queryLog($qry, $level=DB_QUERY_LEVEL,$destination=DB_QUERY_LOG_DESTINATION) {
            if($destination==1) {  // database log
                ...
                if($sessionHandler->getSessionVariable('InstituteId')!='') {
                     ...
                     $q = 'INSERT INTO `query_log`(instituteId,sessionId,fromIp,userId,queryDetail,queryType,logDate)VALUES('.$sessionHandler->getSessionVariable('InstituteId').','.$sessionHandler->getSessionVariable('SessionId').',"'.$_SERVER['REMOTE_ADDR'].'",'.$sessionHandler->getSessionVariable('UserId').', "'.add_slashes($qry).'","'.$queryType.'",NOW())';
