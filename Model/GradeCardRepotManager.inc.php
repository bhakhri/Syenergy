...
//-------------------------------------------------------------------------------       
    ...
    public function getStudentPeriodData($conditions='') {
        ...
        $query = "SELECT
                     DISTINCT sp1.periodName, sp1.studyPeriodId  
                 FROM
                    ".TOTAL_TRANSFERRED_MARKS_TABLE." ttm1, class cc, study_period sp1,
                    (SELECT
                        DISTINCT ttm.studentId
                     FROM
                     ...
                     WHERE
                        ...
                        $conditions) tt
                 WHERE
                      ttm1.studentId = tt.studentId AND
                      ttm1.classId = cc.classId AND
                      sp1.studyPeriodId = cc.studyPeriodId            
                 ORDER BY 
                      sp1.studyPeriodId ";
    ...
    public function getStudentStudyPeriodWiseInfo($conditions='') {
        ...
        $query = "SELECT
                          DISTINCT     
                                ...
                                (SELECT degreeName FROM degree br, class cr, student sr 
                                 WHERE br.degreeId = cr.degreeId AND cr.classId = sr.classId AND sr.studentId = ttm.studentId) AS currDegreeName,
                                (SELECT branchName FROM branch bbr WHERE bbr.branchId = stu.branchId) AS presentBranchName 
                FROM
                        ...
                        subject_to_class f,".TOTAL_TRANSFERRED_MARKS_TABLE." ttm
                        LEFT JOIN grades gr ON (ttm.gradeId = gr.gradeId AND gr.instituteId = $instituteId),
                        `subject` sub LEFT JOIN subject_category cate ON
                        (sub.subjectCategoryId = cate.subjectCategoryId  )
                WHERE
    ...
    public function getStudentGradeCardInfo($conditions) {
        ...
        $query = "SELECT
                        ...
                        IF(gr.gradeLabel IS NULL, 'I', gr.gradeLabel) as gradeLabel1,
                        IF(INSTR(GROUP_CONCAT(DISTINCT CONCAT('~',ttm.marksScoredStatus,'~')),'~D~')>0,'I',
                          IF(INSTR(GROUP_CONCAT(DISTINCT CONCAT('~',ttm.marksScoredStatus,'~')),'~A~')>0,'I',
                            IF(gr.gradeLabel IS NULL, 'I', gr.gradeLabel))) AS gradeLabel, 
                        IF(f.isAlternateSubject='0',sub.subjectCode,sub.alternateSubjectCode) AS subjectCode,
                            IF(f.isAlternateSubject='0',sub.subjectName,sub.alternateSubjectName) AS subjectName,
                  ...
                  FROM
                        student stu, study_period sp, class cls, university c, degree d,  branch e, batch b,
                        subject_to_class f,".TOTAL_UPDATED_MARKS_TABLE." ttm LEFT JOIN grades gr ON (ttm.gradeId = gr.gradeId AND gr.instituteId = $instituteId),
                        `subject` sub LEFT JOIN subject_category cate ON (sub.subjectCategoryId = cate.subjectCategoryId )
                  WHERE
                        f.hasParentCategory = 0 AND  
                        cls.batchId = b.batchId                 AND
                  ...
                  FROM
                        student stu, study_period sp, class cls, university c, degree d,  branch e, batch b, subject_to_class f1, optional_subject_to_class os,
                        student_optional_subject f,".TOTAL_UPDATED_MARKS_TABLE." ttm LEFT JOIN grades gr ON (ttm.gradeId = gr.gradeId AND gr.instituteId = $instituteId),
                        `subject` sub LEFT JOIN subject_category cate ON (sub.subjectCategoryId = cate.subjectCategoryId )
                  WHERE
                        f1.hasParentCategory = 1 AND
                        f1.subjectId = os.parentOfSubjectId     AND
