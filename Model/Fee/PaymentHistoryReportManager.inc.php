...
class PaymentHistoryReportManager {
...
//--------------------------------------------------------
	...
	public function getPaymentHistoryCount($filter){
		$query = "
			SELECT CONCAT(s.firstName,' ',s.lastName) AS studentName, c.className, b.branchName,sp.periodName, s.rollNo,s.regNo, SUM(frd.amount) AS amount, frd.paymentMode ,date_format(frd.receiptDate, '%Y-%m-%d' ) AS receiptDate,frm.feeClassId,s.studentId,frd.receiptNo
	...
	public function getPaymentHistoryCountNew($condition){
		$query = "
			    SELECT 
                    CONCAT(s.firstName,' ',s.lastName) AS studentName, c.className, 
                    b.branchName,sp.periodName, s.rollNo,s.regNo, SUM(frd.amount) AS amount, frd.paymentMode ,
			    ...
			    FROM	
                    `fee_receipt_master` frm,`fee_receipt_details` frd, `student` s, `class` c, `branch` b, `study_period` sp
				WHERE
				    frm.feeReceiptId = frd.feeReceiptId
					AND	frm.feeClassId = frd.classId
					AND	frm.studentId = frd.StudentId
 ...
 public function getPaymentHistoryDetails($filter='',$limit='',$sortOrderBy='',$sortField=''){
      $query = "
            SELECT 
                    ...
                    c.className, b.branchName,sp.periodName, s.rollNo,s.regNo, 
	...
	public function getPaymentHistoryDetailsNew($condition='',$limit='',$sortOrderBy='',$sortField=''){
      $query = "
            SELECT 
                    ...
                    IFNULL(SUM(IF(frd.paymentMode=3,frd.amount,IF(frd.paymentMode=2,frd.amount,0))),'') AS receiveDD,
                    IFNULL(GROUP_CONCAT(DISTINCT CONCAT(bk.bankAbbr,' (',frd.number,' ',DATE_FORMAT(frd.dated,'%d-%b-%y'),')') SEPARATOR ', '),'') AS ddDetail,
                    IFNULL(usr.userName,'') AS userName,frd.isOnlinePayment, 
                    IFNULL(CONCAT(emp.employeeName, ' (',emp.employeeCode,')'),'Admin') AS employeeCodeName,
                    CONCAT(IF(frd.feeType=1,'Academic',IF(frd.feeType=2,'Transport',IF(frd.feeType=3,'Hostel','All'))),' (', 
                    IF(frd.isOnlinePayment=1,'Online Payment',IF(frd.paidAt=1,'Bank','On Desk')),')') AS feeTypeOf,
                       IFNULL(SUM(IF(frd.paymentMode=4,frd.amount,0)),'') AS onlinePayment,
                       CONCAT(IF(frd.feeType=1,'Academic',IF(frd.feeType=2,'Transport',IF(frd.feeType=3,'Hostel','All'))),' (', 
                     IF(frd.isOnlinePayment=1,'Online',''),')') AS feeOnlineTypeOf,frd.receiptNo
             FROM   
                    `fee_receipt_master` frm,`student` s, `class` c, `branch` b, 
                    `study_period` sp, `fee_cycle_new` fcn, 
                    `fee_receipt_details` frd LEFT JOIN `bank` bk ON frd.bankId = bk.bankId
...
//--------------------------------------------------------	
	public function getReceiptDetails($receiptNo,$condition){
		$query = "SELECT 
                        ...
                        frm.studentId, frm.feeClassId, IFNULL(frd.feeType,'') AS feeType
                   FROM    
...
//
//--------------------------------------------------------
	public function getTotalAlreadyPaid($feeReceiptId,$installmentNo,$ttFeeType=''){
		$query = "SELECT sum(amount) AS paidAmount
					FROM	`fee_receipt_details`
					WHERE	feeReceiptId = '$feeReceiptId'                                      
					AND	installmentNo < '$installmentNo'
					AND   feeType = '$ttFeeType'
					AND	isDelete = 0";
