...
//----------------------------------------------------------------------------------------------------------
//THIS FUNCTION IS USED FOR all the reports for pie chart
//
//$conditions :db clauses
// Author :Rajeev Aggarwal 
// Created on : (15.10.2008)
...
//
//------------------------------------------------------------------------------------------------------         
    public function getStudentReportList($conditions='',$orderBy='') {
    
       global $sessionHandler;
       $query = "SELECT firstName,lastName,studentMobileNo,studentEmail,studentGender,DATE_FORMAT(dateOfAdmission, '%d-%b-%y') as dateOfAdmission,DATE_FORMAT(dateOfBirth, '%d-%b-%y') as dateOfBirth,fatherTitle,fatherName,branchName,degreeName ,batchName,periodName,quotaName 
				  FROM student sc, class cls,branch br, degree dr , batch ba , study_period sp, quota qta
				  WHERE 
				  sc.classId = cls.classId AND 
				  cls.branchId = br.branchId AND
				  cls.degreeId = dr.degreeId AND
				  cls.batchId = ba.batchId AND
				  cls.studyPeriodId = sp.studyPeriodId AND
				  sc.quotaId = qta.quotaId
				  $conditions
				  AND cls.sessionId = ".$sessionHandler->getSessionVariable('SessionId')." AND 
				  cls.instituteId =".$sessionHandler->getSessionVariable('InstituteId');
...
//
//------------------------------------------------------------------------------------------------------         
    public function getStudentDegreeWiseList($conditions='',$orderBy='') {
    
       global $sessionHandler;
       $query = "SELECT firstName,lastName,studentMobileNo,studentEmail,studentGender,DATE_FORMAT(dateOfAdmission, '%d-%b-%y') as dateOfAdmission,DATE_FORMAT(dateOfBirth, '%d-%b-%y') as dateOfBirth,fatherTitle,fatherName,degreeName 
				  FROM student sc, class cls, degree dr 
				  WHERE 
				  sc.classId = cls.classId AND 
				   
				  cls.degreeId = dr.degreeId  
...
//------------------------------------------------------------------------------------------------------         
    public function getStudentHostelWiseList($conditions='',$orderBy='') {
       ...
       $query = "SELECT 
                        firstName,lastName,
                        studentMobileNo,studentEmail,
                        studentGender,DATE_FORMAT(dateOfAdmission, '%d-%b-%y') as dateOfAdmission,
                        DATE_FORMAT(dateOfBirth, '%d-%b-%y') as dateOfBirth,
                        fatherTitle,fatherName,
                        h.hostelName,hr.roomName
				  FROM 
                        class cls,student sc
                        LEFT JOIN hostel_students hs ON hs.studentId=sc.studentId
                        LEFT JOIN hostel_room hr ON hr.hostelRoomId=hs.hostelRoomId
